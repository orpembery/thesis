\section[Proofs of Theorems \MakeLowercase{\ref{thm:hh-gen} and \ref{thm:hh-hetero}}]{Proofs of \cref{thm:hh-gen,thm:hh-hetero}}\label{sec:hhproof}
In \cref{sec:placing} we place the Helmholtz stochastic EDP into the framework developed in \cref{sec:gen-framework}. In \cref{sec:hh-cond} we give sufficient conditions for the Helmholtz stochastic EDP to satisfy \namecrefs{con:coeffstoform} \ref{con:coeffstoform}, \ref{con:coeffstofunc}, \ref{con:cborel}, etc.. In \cref{sec:applying} we apply the general theory developed in \cref{sec:gen-framework} to prove \cref{thm:hh-gen,,thm:hh-hetero}.
...
Let
\begin{align*}
  \LtRDR \de \big\{&\fz \in \LtDR \st \supp\mleft(\fz\mright) \compcont \BR\big\},\\
...
\hspace{-3cm}\LiRminDRspd \de \Big\{&\Az \in \LiDRspd \st \supp\mleft(I-\Az\mright) \compcont \BR\Big\},\text{ and}\\
%\end{align*}
%\begin{align*}
&\hspace{-3cm}\WoiRminDRspd \de \set{\Az \in \LiRminDRspd \st\Az \in \WoiDRspd}.
\end{align*}
\ede

Observe that the norms on $\LiDRRR$, $\LiDRRRdtd$, $\WoiDRRRdtd$, and $\LtDR$ induce metrics on $\LiRminDRRR,$ $\LiRDRspd,$ $\WoiRminDRspd,$ and $\LtRDR$ respectively. These spaces are not vector spaces, and are not complete, but completeness and being a vector space is not required in what follows---we only need them to be metric spaces.


\bde[Deterministic form and functional]

Given some $\mleft(\Az,\nz,\fz\mright) \in \LiRDRspd\- \times \LiRminDRRR \times \LtRDR$, let the sesquilinear form $\aGm$ on $\HozDDR \times \HozDDR$  and the antilinear functional $\Lh$ on $\HozDDR$ be given by
\begin{align*}
...
We now restate \cref{prob:vedp} in the notation of this \lcnamecref{chap:stochastic}.

\bprob[Helmholtz EDP]\label{prob:edpstoch}
For $\mleft(\Az,\nz,\fz\mright) \in \LiRDRspd \times \LiRDRRR \times \LtRDR$ find 
...
We let
%\beqs
$\cC \de \WoiRminDRspd \times \LiRminDRRR \times \LtRDR$
%\eeqs
...
&\quad\quad\quad\,\,\leq\NLiDRop{\Am-\Az}\NLtDR{\grad \vo}\NLtDR{\grad \vt} 
\\
...
\item Showing $\NHozDDRas{F}$ depends on each of $\NLtDR{\grad \uo},$ $\NLtDR{\uo},$ $\NLiDRop{\Ao-\Az},$ $\NLiDRRR{\no-\nz},$ and $\NLtD{\fz-\fo}.$
\item Eliminating the dependence on $\uo$ by writing $\uo = \uz-\usd$ and moving terms in $\usd$ to the left-hand side, to obtain a bound on $\usd$ of the form
  \begin{align*}
&\NLtDR{\grad \usd} + k\NLtDR{\usd} \\
&\qquad\leq \Ctilde\Big(\uz,\Az,\nz,\NLiDRop{\Ao-\Az},\NLiDRRR{\no-\nz},\NLtDR{\fz-\fo}\Big).
\end{align*}
\item Concluding that $\usd\rightarrow0$ in $\HozDDR$ as $(\Ao,\no,\fo) \rightarrow (\Az,\nz,\fz)$ in $\cC.$
\een\epf
...
To prove that \cref{con:B} holds for the Helmholtz stochastic EDP, we first state the deterministic analogue of \cref{thm:hh-hetero}. This is essentially a restatement of \cref{thm:edp}, but on the set $\DR$, rather than on $\Dp.$ \Cref{thm:eubedp} uses the notation of \cref{def:NT}.
...
\bth[Well-posedness of the Helmholtz EDP {\cite[Theorem 2.5]{GrPeSp:19}}]\label{thm:eubedp}
...
\noindent If $A \in \LiOLiDRspd,$ $n \in \LiOLiDRRR,$ and $f$ is strongly measurable, then \cref{con:A} holds for the Helmholtz stochastic EDP.
\ele
...
&\hspace{-1cm}\leq \NLiDRop{A(\omega)}  \NLtDR{\grad \vo}\NLtDR{\grad \vt}\\
&\hspace{-1cm}\quad+ k^2 \NLiDRRR{n(\omega)}\NLtDR{\vo}\NLtDR{\vt}+ C(k) \NHhGR{\gamma\vo}\NHhGR{\gamma\vt}.
\end{align*}
%where we have used the fact that the two norms $\NLiDRRRdtd{\cdot}$ and $\NLiDRop{\cdot}$ are equivalent.
...
\beqs
\Nhomspace{\mleft(\toform\circ\coeff\mright)(\omega)}\leq\Ctilde\max\set{\NLiDRop{A(\omega)},\NLiDRRR{n(\omega)},C(k)}\NHokDR{\vo}\NHokDR{\vt}.
\eeqs
