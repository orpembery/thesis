\documentclass[11pt]{book}

\listfiles

%\input{versions}

\input{packages}

\input{macros}

\title{The Helmholtz Equation in Heterogeneous and Random Media: Analysis and Numerics}
\author{Owen Rhys Pembery}
\degree{Doctor of Philosophy}
\department{Department of Mathematical Sciences}
\degreemonthyear{\textls{February 2020}}

\begin{document}

\input{settings}

\maketitle

\makeaftertitle

\input{summary}

%\section*{Acknowledgements}

\tableofcontents

\listoffigures
\listoftables
\listofalgorithms

\chapter{Introduction}\label{chap:intro}
\input{introduction}

\chapter{PDE theory for the deterministic Helmholtz equation and the theory of its finite-element discretisation}\label{chap:background}
\chaptermark{PDE and FE Theory}

\input{background}

\input{fem-background-material}

\subsection{Constants from \cref{sec:fem}}\label{app:constants}
\input{constants}

\section{Summary and future work}
\input{fem-conc}

%\input{discrete-sobolev-spaces}

%\input{new-fem-bounds}

\chapter{Well-posedness of formulations of the sto\-chas\-tic Helmholtz equation}\label{chap:stochastic}
\chaptermark{The Stochastic Helmholtz Equation}

\input{new-introduction} 

\input{helmholtz-results}

\input{generating}

\input{discussion}

\input{general-framework}

\input{proofs-abstract}

\input{proofs-helmholtz}

\input{stochastic-conc}

\chapter[Nearby preconditioning for the Helmholtz equation]{Nearby preconditioning for the\\Helmholtz equation}\label{chap:nbpc}
\chaptermark{Nearby Preconditioning}
\input{nbpc}

\input{nbpc-conc}

\chapter[Monte-Carlo and Multi-Level Monte-Carlo methods for the stochastic Helmholtz equation]{Monte-Carlo and Multi-Level\\Monte-Carlo methods for the Helmholtz equation}\label{chap:mlmc}
\chaptermark{Multi-Level Monte Carlo}

\input{mlmc}

\input{mlmc-conc}

%% \chapter{Conclusion and future work}\label{sec:future}
%% \input{conclusion}

\appendix

\chapter[Failure of Fredholm theory]{Failure of Fredholm theory for a sto\-chas\-tic variational formulation of Helmholtz problems}\label{sec:federico}
\chaptermark{Failure of Fredholm Theory for stochastic Helmholtz problems}
\input{federico} 	

\chapter{Recap of basic material on measure theory and Bochner spaces}\label{app:mtBs}
\chaptermark{Recap of measure theory and Bochner spaces}
\input{review-material}

\chapter{Measurability of series expansions (used in \cref{sec:generating})}\label{app:meas}
\chaptermark{Measurability of series expansions}
\input{measurable-sums}

\chapter[Error estimators for complex random variables]{Error estimators for complex-valued random variables}\label{app:complexerror}
\input{complex-error-estimators}

\chapter[Numerical investigation of QMC]{Numerical investigation of QMC convergence for the Helmholtz equation}\label{app:hhqmcconv}
\input{qmc-convergence-plots}

\chapter[Additional probabilistic results]{Additional probabilistic results for nearby preconditioning}\label{app:probnbpc}
\input{prob-nbpc}

\chapter{Computational set-up}\label{app:compsetup}
\input{comp-setup}

\bibliographystyle{plainnat}
\bibliography{biblio_thesis}

\end{document}
