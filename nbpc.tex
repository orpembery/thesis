...
\section{Introduction and Motivation from UQ}\label{sec:intro}
...
For a proof that the EDP solution (as defined by Definition \ref{def:EDP}) is the truncation to $\Omega_R$ of the solution of \eqref{eq:pde} posed in $\Dp$ in a weak sense, and satisfying both the Sommerfeld radiation condition \eqref{eq:src} and the Dirichlet boundary condition $\gamma u =0$ on $\Gamma$, see, e.g., \cite[Lemma 3.3]{GrPeSp:19}.
\ere
...
%\eeq
%where $\|\cdot\|_{\HokDR}$ is defined by \eqref{eq:1knorm}.
%\item For a proof that the EDP solution (as defined by Definition \ref{def:EDP}) is the truncation to $\Omega_R$ of the solution of \eqref{eq:pde} posed in $\Dp$ in a weak sense, and satisfying both the Sommerfeld radiation condition \eqref{eq:src} and the Dirichlet boundary condition $\gamma u =0$ on $\Gamma$, see, e.g., \cite[Lemma 3.3]{GrPeSp:19}.
...
\eeq
This property follows from the fact that, if $u$ and $v$ are solutions of the homogeneous Helmholtz equation $\Delta u +k^2 u=0$ in $\RRd\setminus \overline{B_R}$, both satisfying the Sommerfeld radiation condition \eqref{eq:src}, then
\beqs
...
% 
%If Condition \ref{cond:1} holds and $u\in H^2$, one can show that $\|u\|_{H^2(\DR)}\lesssim k\N{f}_{\LtDR}$, and then, using results about polynomial approximation theory, we have
% \beq\label{eq:AN1}
%\N{u-u_h}_{\HokDR} \lesssim h \N{u}_{H^2(\DR)} + hk \N{u}_{H^1(\DR)}\lesssim hk \N{f}_{\LtDR} \lesssim \frac{1}{k}\N{f}_{\LtDR},
%\eeq
%and so Part (i) of Condition \ref{cond:2} is readily satisfied.
