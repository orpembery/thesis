...
Moreover, the cost of \emph{constructing} preconditioners for the resulting linear systems is also very expensive. For example, if our preconditioner is given by an exact LU factorisation, then (in 3-d) the cost of calculating the preconditioner is $\cO\mleft(N^2\mright)$ and the cost of applying the preconditioner is $\cO\mleft(N^{4/3}\mright)$ (where $N$ is the number of unknowns), see, e.g., \cite[Section 4]{GaZh:19}\optodo{See if any of the sparse matrix books have this in.}.
...
Numerical experiments in \cref{sec:num} indicate that the condition \cref{eq:cond} is sharp, in that the $k$ in \cref{eq:cond} cannot be replaced by $k^\alpha$ for $\alpha<1$. This suspected sharpness of \cref{eq:cond} is also supported by the PDE-result \cref{thm:2} below. Indeed, \cref{thm:2} % and \cref{lem:1} 
...
It is unsurprising that the condition \cref{eq:sufficientlysmall} is a sufficient condition to answer both Q1 and Q2. Recall that $1/k$ is proportional to the wavelength $2\pi/k$ of the wave $u$ (at least when $A=I$ and $n=1$). As the wavelength is the natural length scale associated with the wave $u$, it is unsurprising that perturbations of size (up to) $1/k$ give bounded relative difference (in Q2) and bounded GMRES iterations for the nearby-preconditioned linear system (in Q1). In some sense, perturbations of size up to $1/k$ are `unseen' by the PDE.
\ere
...
\input{nbpc-linfinity-A-0.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5$}\label{fig:linfinityA0}
\end{figure}

\begin{figure}
\input{nbpc-linfinity-A-1.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5/k^{1/2}$}\label{fig:linfinityA1}
\end{figure}

\begin{figure}
\input{nbpc-linfinity-A-2.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5/k$}\label{fig:linfinityA2}
\end{figure}

\begin{figure}
\input{nbpc-linfinity-n-0.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5$}\label{fig:linfinityn0}
\end{figure}

\begin{figure}
\input{nbpc-linfinity-n-1.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5/k^{1/2}$}\label{fig:linfinityn1}
\end{figure}

\begin{figure}
\input{nbpc-linfinity-n-2.pgf}
  \caption{GMRES iteration counts for $\AmatoI\Amatt$ given by ... where $\alpha = 0.5/k$}\label{fig:linfinityn2}
\end{figure}
...
If $\Vhp$ is quasi-uniform with a nodal basis, then
...
For a proof of \cref{lem:normequiv}, see \cite[Chapter V, Lemma 2.5]{Br:07}.\ednote{Euan---in \cite[Chapter V, Lemma 2.5]{Br:07} this result is written slightly differently (with scaled basis functions and a scaled 2-norm) but it's equivalent to our result. I presume I don't need to put anything 'tranlating' their result to ours? (Can download the 3rd edition from the CUP website if you use your Bath institutional login.)}

%% \bpf[Sketch proof of \cref{lem:normequiv}]\optodo{can omit this if can find a good reference. One possibility . Need to check basis scaling business.}
...
%%   \centering
%%   \input{G-20.pgf}
%%   \caption{Plot of $\Gfnname$ for $\NLiDRRR{\no-\nt} \in \mleft(0.01,1.0\mright)$, for $k=20,$ $\Ct = 0.1,$ $N=k^3,$ and $\eps = 10^{-5}$.\label{fig:G20}}
%%     \end{figure}

%% \begin{figure}
%%   \centering
%%   \input{G-40.pgf}
%%   \caption{Plot of $\Gfnname$ for $\NLiDRRR{\no-\nt} \in \mleft(0.01,1.0\mright)$, for $k=40,$ $\Ct = 0.1,$ $N=k^3,$ and $\eps = 10^{-5}$.\label{fig:G40}}
%%     \end{figure}

%% \begin{figure}
%%   \centering
%%   \input{G-100.pgf}
%%   \caption{Plot of $\Gfnname$ for $\NLiDRRR{\no-\nt} \in \mleft(0.01,1.0\mright)$, for $k=100,$ $\Ct = 0.1,$ $N=k^3,$ and $\eps = 10^{-5}$.\label{fig:G100}}
%%     \end{figure}
...
\begin{figure}
  \centering
  \input{G.pgf}
  \caption{Plot of $\Gfnname$ for $\NLiDRRR{\no-\nt} \in \mleft(0.01,1.0\mright)$, for $k=20,40,100$ $\Ct = 0.1,$ $N=k^3,$ and $\eps = 10^{-5}$.\label{fig:G}}
    \end{figure}
...
All of the operations used in constructing the vectors $\bxm$ in the GMRES algorithm are measurable functions of $bBxmmo$ and $\AmatoI\Amatt$ (see, e.g., \cite[Algorithms 11.4.2 and 5.1.3]{GoVa:13}), therefore $\mleft(\bxm\mright)_{m=1}^N$ is a sequence of measurable functions, a stochastic process\optodo{Check formal definition}. The stopping criterion $\Nt{\bxm} < \eps$ is a \defn{stopping time} in the theory of stochastic processes\optodo{Ref, check} (because at time $m$ one can evaluate whether the criterion has been fulfilled. Therefore, in particular\optodo{Reasoning correct?} the number of GMRES iterations after which the criterion is fulfilled (i.e., $\GMRES{\eps}{\no}{\nt}$) is a random variable\optodo{Ref}.\optodo{Ask a probabalist about this.}
\ere
...
In our numerical experiments (for which we use the IIP, with $h = k^{-3/2}$ on the unit square\optodo{Standard computational setup in appendix}) we consider three cases:
\ben
...
\caption{Plots of number of GMRES iterations required for convergence of ... with the standard deviation $=1 $ according to \cref{thm:probgmres}.\label{fig:prob-theory-plot-0.0}}
\end{figure}

\begin{figure}[h]
    \centering
\input{prob-gmres-theory-1.0.pgf}
\caption{Plots of number of GMRES iterations required for convergence of ... with the standard deviation $=1/k $ according to \cref{thm:probgmres}.\label{fig:prob-theory-plot-1.0}}
\end{figure}

\begin{figure}[h]
    \centering
    \input{prob-gmres-theory-2.0.pgf}
    \caption{Plots of number of GMRES iterations required for convergence of ... with the standard deviation $=1/k^{2} $ according to \cref{thm:probgmres}.\label{fig:prob-theory-plot-2.0}}
\end{figure}
\begin{figure}[h]
    \centering
\input{prob-plot-rate-0.0.pgf}
\caption{Plots of number of GMRES iterations required for convergence for 1000 realisations of ... with the standard deviation $=1 $  \label{fig:prob-plot-0.0}}
\end{figure}

\begin{figure}[h]
    \centering
\input{prob-plot-rate-1.0.pgf}
\caption{Plots of number of GMRES iterations required for convergence for 1000 realisations of ... with the standard deviation $=1/k $  \label{fig:prob-plot-1.0}}
\end{figure}

\begin{figure}[h]
    \centering
\input{prob-plot-rate-2.0.pgf}
\caption{Plots of number of GMRES iterations required for convergence for 1000 realisations of ... with the standard deviation $=1/k^2 $  \label{fig:prob-plot-2.0}}
\end{figure}
\optodo{Sort captions}
...
Having defined the randomly shifted QMC rule, one can the use the standard unbiased statistical estimator of the QMC standard error \cite[Equation (4.6)]{GrKuNuScSl:11}\optodo{Ref, ref to appendix}
\beqs
...
\end{figure}\optodo{Refs for QMC error eqn}
\optodo{Fix plots with $e$ on x axis.}
...
We use these computational results to determine how $\NQMC$ should increase with $k$, in order to keep the QMC error bounded. Based on QMC theory results for the stationary diffusion equation\optodo{ref results}, we make the assumption that the QMC error (with one shift) satisfies
