...
%\beq\label{eq:Dk3}
%\N{\vvec}_{\Dmat_k}^2\de   \N{v_h}^2_{\HokDR}=\big( \Dmat_k \vvec,\vvec\big)_2,
%\eeq
%for
%$v_h =\sum_i v_i \phi_i$.
...
m_- h^{d/2} \N{\vvec}_2 \leq \N{v_h}_{\LtDR} \leq m_+ h^{d/2} \N{\vvec}_2,
\eeq
and
\beq\label{eq:normequiv2}
s_- h^{d/2} \N{\vvec}_2 \leq \N{\nabla v_h}_{\LtDR} \leq s_+ h^{d/2-1} \N{\vvec}_2,
\eeq
...
\beq\label{eq:bound4}
\N{u-u_h}_{\HokDR} \leq C^{(1)}_{\rm FEM2}\,k\, \N{\LE}_{\HokDRs} \quad\tfa k\geq k_0, 
\eeq
where $C^{(1)}_{\rm FEM2}$  is independent of $k$ and $h$, but dependent on $\Aso, \nso, \Dm, R, k_0$, and $p$.
\end{condition}

For details of when \cref{cond:1nbpc,cond:2nbpc} are satisfied, see \cref{sec:pdetheory} (for \cref{cond:1nbpc}) and \cref{sec:helmfedisc} (for \cref{cond:2nbpc}).

\bre[\Cref{eq:bound4} has the same $k$-dependence as \cref{eq:bound3}]\label{rem:yesitis}
Note that \cref{eq:bound4} has the same $k$-dependence as \cref{eq:bound3} despite the fact that a factor $k$ appears on the right-hand side. If $\LE(v) = \int_{\DR} f\vbar,$ then
\beqs
\NHokDRs{\LE} = \sup_{v \in \HozDDR} \frac{\abs{\LE(v)}}{\NHokDR{v}} \leq\sup_{v \in \HozDDR} \frac{\NLtDR{f}\NLtDR{v}}{\NHokDR{vz}} \lesssim \frac1k \sup_{v \in \HozDDR} \frac{\NLtDR{f}\NLtDR{v}}{\NLtDR{vz}} = \frac{\NLtDR{f}}k.
\eeqs
The factor $k$ appears in \cref{eq:bound4} because the weighted norm $\NHokDR{\cdot}$ is used in the definition of $\NHokDRs{\cdot},$ rather than the unweighted norm $\NHoDR{\cdot}.$
\ere
...
Given $\fvec \in \CC^N$ and $n\in \LiDRRR$, we create a variational problem whose Galerkin discretisation leads to the equation $\Amato \tbu = \Mmat_n\,\fvec$.
...
%& \leq\big(C^{(1)}_{\rm FEM1}+  C^{(1)}_{\rm bound}\big)\N{n}_{L^\infty(\DR)} m_+ h^{d/2} \N{\fvec}_2,
\end{align}
...
\,\tand\,
\big\|\widetilde{f}\big\|_{\LtDR} \leq m_+ h^{d/2}\N{\fvec}_2,
\eeqs
...
In a similar way to the proof of \cref{lem:keylemma1}, given $\fvec \in \CC^N$ and a symmetric $A\in L^\infty(\DR, \RR^{d\times d})$, let $\widetilde{f} \de \sum_j f_j \phi_j$ and observe that $\widetilde{f} \in \HozDDR$. Define $\widetilde{u}$ to be the solution of the variational problem 
\beq\label{eq:411a}
...
%&\leq \left[ C^{(1)}_{\rm FEM2} k + 
%\frac{1}{\min\{\Asomin,\nsomin\}}\left( 1 + 2 C^{(1)}_{\rm bound}\nsomaxk  \right) 
%\right]\big\|A\big\|_{L^\infty(\DR)}s_+ h^{(d-2)/2} \N{\fvec}_2,
\end{align}
...
m_- h^{d/2} k \N{\tbu}_2 \leq k \N{\widetilde{u}_h}_{\LtDR} \leq \N{\widetilde{u}_h}_{\HokDR}
\,\tand\,
\big\|\nabla \widetilde{f}\big\|_{\LtDR} \leq s_+ h^{d/2-1}\N{\fvec}_2,
\eeqs
...
%\beq\label{eq:Dk}
%\N{\vvec}_{\Dmat_k}^2\de \big( (\Smat_I + k^2 \Mmat_1)\vvec,\vvec\big)_2 = \N{v_h}^2_{\HokDR}
%\eeq
%for
%$v_h =\sum_i v_i \phi_i$. 
...
Observe that \cref{eq:obs1} proves the sharpness of the nontrapping resolvent estimate \cref{eq:bound1}, since both the $L^2(\DR)$ norm of $\widetilde{f}$ and the $\HokDR$ norm of $e^{i kr}\chi(r)$ are proportional to $k$, and hence each to other (see, e.g., \cite[Lemma 3.10]{ChMo:08},  \cite[Lemma 4.12]{Sp:14}). Also, recall that the proof of the PDE bound \cref{eq:PDEbound} used the nontrapping resolvent estimate \cref{eq:bound1} applied to $\uso-\ust.$ Therefore we expect that if we set things up so that
