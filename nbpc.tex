...
\Gfn{\NLiDR{\no-\nt}} =
\begin{dcases}
\max\set{N,\frac{\log{\eps}}{\log\mleft(\frac{2\alpha^{1/2}}{\mleft(1+\alpha\mright)^2}\mright)}+1} & \tif \alpha < 1\\
N & \tif \alpha \geq 1,
\end{dcases}
\eeq
...
\begin{dcases}
\leq \PP\mleft(\NLiDRRR{\no-\nt} \leq \GfnI{R}\mright) & \tif R < \dofs,\\
=1 & \tif R = \dofs.
\end{dcases}
\end{align*}
\enth
...
For $R < \dofs,$ the proof is immediate from \cref{lem:probgmres1,lem:gtechnical}. For $R = \dofs$, the proof is immediate from\optodo{At most dofs result.}.
\epf
