...
However, there are three immediate questions with the above approach:
\ben
\item The above procedure assumes we know the radius of the balls we wish to construct, and then returns the total number of preconditioning points, and their locations.
\item There is no guarantee that the numbers of points $\Nj$ calculated above are integers.
  \item There is no guarantee the preconditioning points given by the above procedure are QMC points.
    ...
    Recall that we assume that we are given a target number of preconditioners $\Npretarget$. The above procedure (amongst other things) defines a map $\RRp \rightarrow \RRp$ given by $r \mapsto \Npre.$ Therefore we can calculate numerically the value $\rideal$ such that $\rideal \mapsto \Npretarget.$ (In the code\optodo{ref}, we do this via interval bisection.) Once we know $\rideal,$ we can calculate the numbers of centres in each dimension $\Npreidealo,\ldots,\NpreidealJ$ as above (recalling that the $\Npreidealj$ are not necessarily integers). We can then round all the $\Npreidealj$ to obtain integers (as $\Npreidealj \geq 1$ for all $j$ by construction) $\Npreactualj$, and we then take $\Npreactualj$ centres in each dimension. We can then form the centres as above, giving a total of $\Npreactual = \Npreactualo \times \cdots \times \NpreactualJ$ peconditioning points.

    These points may not be QMC points; if we desired, we could simply calculate the preconditioners at these non-QMC points, however, instead we elect to calculate the preconditioners at QMC points, and so we simply replace each calculated centre with its nearest QMC point.
    \optodo{THis almost certainly needs more writing and more notation}
