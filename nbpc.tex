...
We first show \cref{eq:normequiv1} by direct computation, before concluding \cref{eq:normequiv2} from \cref{eq:normequiv1} and a standard inverse inequality. Throughout this proof, when we use the notation $\sim$ the hidden constants are independent of $\tau \in \Th,$ the mesh size $h$, and $\vh \in \Vhp,$ but may depend on $p.$

For any $\vh \in \Vhp$ we have (letting $\bnj$ denote the nodes of $\Th$)
\begin{align}
...
If $\LG$ is as in \cref{prob:vgen}, $u$ is the solution to the variational problem, the boundary operaor $\T$ satisfies
\beq\label{eq:DPconj}
...
a(\overline{u},v)= \overline{\LG(\overline{v})} \quad\tfa v\in \HozDD.
\eeq
\ele
...
\beqs
\NHokDs{\LG} = \sup_{v \in \HozDD} \frac{\abs{\LG(v)}}{\NHokD{v}} \leq\sup_{v \in \HozDD} \frac{\NLtD{f}\NLtD{v}}{\NHokD{vz}} \lesssim \frac1k \sup_{v \in \HozDD} \frac{\NLtD{f}\NLtD{v}}{\NLtD{vz}} = \frac{\NLtD{f}}k.
\eeqs
...
a^{(1)}(\widetilde{u},v)=\widetilde{\LG}(v) \,\, \tfa v\in H^1_{0,D}(D).
\eeqs
...
\beq\label{eq:411a}
a^{(1)}(\widetilde{u},v)= \widetilde{\LG}(v) \quad\text{ for all } v\in H^1(\Omega),
\quad\text{ where } \quad
 \widetilde{\LG}(v) \de(A\nabla\widetilde{f},\nabla v)_{L^2(\Omega)}.
\eeq
...
\beq\label{eq:41a}
a^{(1)}(\tu_h,v_h)= \widetilde{\LG}(v_h) \quad\text{ for all } v_h\in \Vhp,
\eeq
...
By the assumptions on $\Aso,\, \Ast,\, \nso,\,\st$, and $\Dm$, $u^{(1)}$ and $u^{(2)}$ exist, are unique, satisfy $a^{(1)}(u^{(1)}, v) = \LG(v)$ for all $v \in \HozDD$ and $a^{(2)}(u^{(2)}, v) = \LG(v)$ respectively, where $\LG$ is given by \cref{eq:Ledp}. Subtracting these equations, we find that $u^{(1)}- u^{(2)}$ satisfies the variational problem
\beq\label{eq:vp1}
...
\beqs
\big\|\widetilde{\LG}\big\|_{(\HokD)^*}\leq \max\Big\{\big\|\Aso-\Ast\big\|_{L^\infty(D)}\,,\, \big\|\nso-\nst\big\|_{L^\infty(D)}\Big\}
\big\| u^{(2)}\big\|_{\HokD}.
\eeqs
...
We now describe numerical experiments demonstrating the effectiveness of nearby preconditioning for tackling Helmholtz problems. For the computational setup, including the algorithm we use to generate our QMC points, see \cref{app:compsetup}.
