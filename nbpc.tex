...
\optodo{Ensure consistent notatyion everywhere}
\optodo{Ref to definitions above}
\bre[The EDP with data in $(\HokDR)'$]
In \cref{def:EDP} we defined the EDP with the antilinear functional $F$ arising from an $f\in \LtDR$. In the rest of the \lcnamecref{chap:nbpc}, 
...
\optodo{Check not defined earlier}
Let $(\Vhp)_{h>0}$ be a family of finite-dimensional, nested subspaces of $\HozDDR$, whose union is dense in $\HozDDR$. Furthermore let $\Vhp$ consist of piecewise-polynomials on a simplicial mesh with mesh-size $h$
...
and fixed polynomial degree $p$. (Note that the dimension $N$ of $\Vhp$ then satisfies $N\sim h^{-d}$.)
...
%\quad 
a(u_h,v_h)=F(v_h) \quad \tfa v_h \in \Vhp.
\eeq
\end{definition}

%Definition of Galerkin method

Let $\{\phi_i, i= 1, \ldots, N\}$ be a basis for $\Vhp$ with each $\phi_i$ \emph{real-valued}.
Let 
...
We recall the definition of quasi-uniformity (c.f. \cite[Definition 4.4.13]):
...
\optodo{Move this earlier and ref here?}
...
\eeq
This property follows from the fact that, if $u$ and $v$ are solutions of the homogeneous Helmholtz equation $\Delta u +k^2 u=0$ in $\RRd\setminus \overline{\BR}$, both satisfying the Sommerfeld radiation condition \cref{eq:srcnbpc}, then
\beqs
...
\optodo{reformulate in terms of accuracy}
\begin{condition}[$k$-independent accuracy of the FE solution for $a^{(1)}(\cdot,\cdot)$]
\label{cond:2}

\

(i) Given $k_0>0$, $h$ and $p$ are such that, given $f\in \LtDR$ with $\supp\, f \subset \BR$, the solution $u_h$ of the Galerkin method \cref{eq:Galerkin} with $a(\cdot,\cdot)=a^{(1)}(\cdot,\cdot)$ (and with $F(v)$ defined in \cref{eq:EDPa}) exists and is unique for all $k\geq k_0$. % (so that the matrix $\Amato$ is invertible). 
...
$\Sremaining \defined \SQMC\setminus\set{\bycurrent}$\;
$\bycurrent \defined$ \Nearest{$\bypre,\Sremaining$}\;
}
\Else{
