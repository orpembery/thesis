...
for the Helmholtz equation \cite{GrSpVa:17,GrSpZo:18}, and for the time-harmonic Maxwell equations \cite{BoDoGrSpTo:19}. 
...
The numerical experiments in this section seek to verify the analogues of \cref{thm:1,cor:1} for \cref{prob:vtedp}, and investigate their sharpness. More specifically, the experiments seek to verify whether the condition \cref{eq:cond} is:
\ben
\item sufficient, and
\item necessary
  \een
...
(ii) Given $k_0>0$, $h$ and $p$ are such that, given $\LG\in \HozDDs$, the solution $u_h$ of the Galerkin method \cref{eq:galerkin} 
with $\aG = \aGo$
