...
Let $\nso \in \LiDRRR$ be fixed, and let $\nst:\Omega\rightarrow\LiDRRR$ be a random field. Let $\eps, \dofs, \Nt{\brz}$ be as in \cref{lem:probgmres1}, and let $\Aso = \Ast = I.$ Let $\Amatt(\omega)$ denote the (random) Galerkin matrix corresponding to the discretisation of \cref{prob:edp}\optodo{Change?} with coefficients $\Aso=I$ and $\nst(\omega)$.

Fix $ \in [0,\dofs].$ Then
\begin{align*}
\PP\mleft(\text{GMRES applied to } \AmatoI\Amatt \text{ converges to within a tolerance } \eps \text{ within } R \text{ iterations}\mright)\\
\begin{dcases}
\leq \PP\mleft(\Gfn{\NLiDRRR{\nso-\nst}} \leq R\mright) & \tif R < \dofs,\\
=1 & \tif R = \dofs.
\end{dcases}
\end{align*}
\enth
...
\subsection{Application to QMC methods for the Helmholtz equation}
