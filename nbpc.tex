...
\beqs  
\cK^m(\Cmat, \br^0) := \mathrm{span}\big\{\matrixC^j \br^0 : j = 0, \ldots, m-1\big\}.
\eeqs
...
\cos \beta := \frac{\mathrm{dist}\big(\bzero, W(\matrixC)\big)}{\N{\matrixC}_{2}}.
\eeq
...
\beq\label{eq:obs3}
u^{(2)}(\bx):= -\frac{1}{k^2 c(k)}\frac{\widetilde{f}(r)}{\widetilde{\chi}(r)}
\eeq
and
\beqs
f(\bx):= -\big(\Delta +k^2 \nst(\bx)\big) u^{(2)}(\bx).
...
\ms  \leq\frac{\log{\eps}}{\log\mleft(\frac{2\alpha^{1/2}}{\mleft(1+\alpha\mright)^2}\mright)}+1.
\eeq
...
\beqs
\ms  =\ceil{\frac{\log{\eps}}{\log\mleft(\frac{2\alpha^{1/2}}{\mleft(1+\alpha\mright)^2}\mright)}}.
\eeqs
