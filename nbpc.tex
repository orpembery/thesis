...
\EXP{Q(u)} \approx \frac1N \sum_{l=1}^N Q(u(\omega^{l})),
\eeq
where the $\omega^l$ are elements of the sample space $\Omega.$ To calculate the right-hand side of \cref{eq:samplingexp},  one must solve many deterministic Helmholtz problems, corresponding to different samples $\omega^l$, i.e. corresponding to different realisations of the coefficients $A(\omega^l,\cdot)$ and $n(\omega^l,\cdot)$.
...
Therefore, in this \lcnamecref{chap:nbpc} we consider whether one can reuse the preconditioner from one solve for subsequent solves, and thereby reduce the overall computational effort. This idea of reusing preconditioners for `nearby' (in some sense) linear systems is the `nearby preconditioning' strategy proposed in this \lcnamecref{chap:nbpc}. To analyse this strategy rigorously, we first consider the following problem and question.
...
As described in \cref{sec:wpdisc}, the PDE analysis of the Helmholtz equation naturally takes place in the norm $\NHokD{\cdot}$, and \cref{eq:Dk} shows that the norm $\NDmatk{\cdot}$ is
the discrete analogue of the norm $\NHokD{\cdot}$. %See \cref{eq:Dk2} and \cref{eq:Dk3} below for this norm expressed in terms of the Euc
...
Assume that $\Dm$, $\Aso$, and $\nso$ satisfy \cref{cond:1nbpc} below, and $h$ and $p$ satisfy \cref{cond:2} below. Given $k_0>0$, there exist constants $\Co$ and $\Ct$  independent of $h$ and $k$ (but dependent on $\Dm, \Aso, \nso, p$, and $\kz$) such that if 
% there exists $C_2>0$, independent of $h$ and $k$ (but dependent on $\Dm, \Aso, \nso$, $p$, and $k_0$) and given explicitly in \cref{eq:C2} below,
% such that if 
\beq\label{eq:cond}
C_1 \,k \,\NLiDRRdtd{\Aso-\Ast} +C_2 \, k\, \NLiDR{\nso-\nst}
\leq \frac{1}{2}
\eeq
...
The constants $\Co$ and $\Ct$ are given explicitly  in \cref{eq:C1nbpc,eq:C2} below.

\bth[Answer to \cref{it:nbpcq1}: $k$-independent (unweighted) GMRES iterations]\label{cor:1a}
Assume that $\Dm$, $\Aso$, and $\nso$ satisfy \cref{cond:1nbpc} below, and $h$ and $p$ satisfy \cref{cond:2} below. Given $k_0>0$,
...
\NLiDRRdtd{\Aso-\Ast} + C_2 \, \left(\frac{m_+}{m_-} \right)k \, \NLiDR{\nso-\nst}
\leq \frac{1}{2}
\eeq
...
Two notes regarding \cref{cor:1,cor:1a}: (i)The $L^\infty$ norms of $\Ao-\At$ and $\no-\nt$ in \cref{cor:1,cor:1a} can be replaced by weaker norms, and the price of making the conditions \cref{eq:cond,eq:conda} more restrictive; see \cref{sec:weaknorm} for more details. (ii) The $L^\infty(D)$ norm on a matrix-valued functions appearing on the right-hand sides of \cref{eq:main1} and \cref{eq:main1a} is defined by
\beqs
\NLiDRRdtd{A}\de \esssup_{\bx\in D}\N{A(\bx)}_2.
\eeqs
(iii) The factor $1/2$ on the right-hand sides of \cref{eq:cond} and \cref{eq:conda} can be replaced by any number $<1$ and the result still holds, although the number of GMRES iterations is then larger -- but still independent of $k$.
...
k\,
\NLiDRRdtd{\Aso-\Ast} \quad\text{ and } \quad k\,\NLiDR{\nso-\nst}
%\Big) 
...
\N{u^{(2)}}_{\HokD}
}\leq C_3 \,k\, \max\set{\NLiDRRdtd{\Aso-\Ast}\,,\, \NLiDR{\nso-\nst}}
\eeq
for all $k\geq k_0$. 
\end{theorem}
...
It is perhaps unsurprising that the condition \cref{eq:sufficientlysmall} is a sufficient condition to answer both \cref{it:nbpcq1} and \cref{it:nbpcq2}. Recall that $1/k$ is proportional to the wavelength $2\pi/k$ of the wave $u$ (at least when $A=I$ and $n=1$). As the wavelength is the natural length scale associated with the wave $u$ one expects perturbations of size up to $1/k$ to be `unseen' by the PDE or numerical method. This is exactly what we see; perturbations of size (up to) $1/k$ give bounded relative difference (in \cref{it:nbpcq2}) and bounded GMRES iterations for the nearby-preconditioned linear system (in \cref{it:nbpcq1}). Also, on a PDE leve, perturbations of order $1/k$ being `unseen' by the PDE can also be seen in bounds proved for $u$ where $n = \no + \eta,$ with $\no$ nontrapping and $\NLiDRR{\eta} \lesssim 1/k,$ see \cref{rem:kdep} above.
\ere
...
We perform experiments for three different forms of $\alpha$  taking $\alpha = 0.5 \times k^\beta$ for $\beta \in 0,0.1,\ldots,0.9,1.$ We expect that when $\beta \neq 1$  the number of GMRES iterations required for convergence will increase as $k$ increases, whereas we expect that when $\beta = 1$ the number of GMRES iterations required for convergence will remain bounded as $k$ increases, even though this behaviour has only been proven for $\NLiDRRdtd{\Aso-\Ast}$ for weighted GMRES.


 When $\beta in \set{0,\ldots,0.3}$ (for $\NLiDRRdtd{\Aso-\Ast}$) and $\beta \in \set{0,\ldots,0.5}$ (for $\NLiDRR{\nso-\nst}$) we see growth in the maximum number of GMRES iterations needed (over all realisations) to achieve convergence, otherwise we see that the number of GMRES iterations is bounded as $k$ increases. This behaviour is better than expected; as the number of GMRES iterations is apparently bounded for a range of $\beta < 1.$ However, we note that this behaviour could be (i) because we are in a pre-asymptotic regime, and the number of GMRES iterations would grow if we increased $k$ further, or (ii) the particular structure of $\nst$ (being piecewise constant, with the pieces independently, randomly chosen) could result in some kind of `averaging' behaviour, meaning the preconditioner is better than would otherwise be expected.

    \begin{figure}
      \centering
\input{nbpc-linfinity-plot-A-0.pgf}
  \caption{Maximum GMRES iteration counts for $\AmatoI\Amatt$ where $\nso=\nst=1$ and $\NLiDRR{\Aso-\Ast} = 0.5\times  k^{\beta}$ for $\beta = 0,0.1,0.2,0.3.$}\label{fig:linfinityA0}
    \end{figure}
    
    \begin{figure}
      \centering
\input{nbpc-linfinity-plot-A-1.pgf}
   \caption{Maximum GMRES iteration counts for $\AmatoI\Amatt$ where $\nso=\nst=1$ and $\NLiDRR{\Aso-\Ast} = 0.5\times  k^{\beta}$ for $\beta = 0.4,0.5 ,0.6,0.7.$}\label{fig:linfinityA1}
\end{figure}

    \begin{figure}
      \centering
\input{nbpc-linfinity-plot-A-2.pgf}
    \caption{Maximum GMRES iteration counts for $\AmatoI\Amatt$ where $\nso=\nst=1$ and $\NLiDRR{\Aso-\Ast} = 0.5\times  k^{\beta}$ for $\beta = 0.8,0.9,1.$}\label{fig:linfinityA2}
\end{figure}

    \begin{figure}
      \centering
\input{nbpc-linfinity-plot-n-0.pgf}
  \caption{Maximum GMRES iteration counts for $\AmatoI\Amatt$ where $\Aso=\Ast=1$ and $\NLiDRR{\nso-\nst} = 0.5\times  k^{\beta}$ for $\beta = 0,0.1,0.2,0.3.$}\label{fig:linfinityn0}
    \end{figure}
...
\eeq
where $\T:\HhGI\rightarrow \HmhGI$ is a bounded linear map and  $\DPGI{\cdot}{\cdot}$ is the duality pairing on $\GI,$ and $\LG  \in \HozDDs.$
\eprob
...
For the remainder of this \lcnamecref{chap:nbpc}, we let $(\Vhp)_{h>0}$ be a family of finite-dimensional, nested subspaces of $\HozDD$, whose union is dense in $\HozDD$. More specifically, we let $\Vhp$ consist of piecewise-polynomials on a quasi-uniform simplicial mesh $\cTh$ with mesh-size $h$
...
and fixed polynomial degree $p$. (Note that the dimension $N$ of $\Vhp$ then satisfies $N\sim h^{-d}$.) As in \cref{rem:crimes} above, we ignore any variational crimes resulting from this discretisation. We now define the finite-element approximation of \cref{prob:vgen}.
\bprob[Finite-element approximation of \cref{prob:vgen}]\label{prob:fevgen}
We say that $\uh \in \Vhp$ is the \defn{finite-element approximation of $u$} (the solution to \cref{prob:vgen}if
...
\beqs
(\Mmat_1 \vvec,\vvec)_2 \sim h^d \N{\vvec}^2_2 \quad\tand\quad h^{d}\N{\vvec}^2_2 \lesssim (\Smat_I \vvec,\vvec)_2 \lesssim h^{d-2} \N{\vvec}^2_2.
\eeqs
...
Let $D$, $A$, and $n$, be as in \cref{prob:vgen}. The adjoint sesquilinear form, $\aGs(\cdot,\cdot)$, to $\aG(\cdot,\cdot)$ defined in \cref{eq:aedp} is given by
\beq\label{eq:EDPadjoint}
\aGs(\vo,\vt) \de \int_{D} 
\Big((A \grad \vo)\cdot\grad \vtb
 - k^2 n \vo\vtb\Big) -  \DPGI{\gamma \vo}{\T(\gamma \vt)}.
\eeq
\end{definition}
...
\eeq
(where $^*$ denotes conjugate transpose) is the Galerkin matrix for the sesquilinear form $\aGs(\cdot,\cdot)$; i.e.~$(\Amat^*)_{ij} = \aGs(\phi_j, \phi_i)$.

\ble[Link between variational problems involving $\aG(\cdot,\cdot)$ and $\aGs(\cdot,\cdot)$]\label{lem:adjoint}
If $\LE$ is as in \cref{prob:vgen}, $u$ is the solution to the variational problem,
\beq\label{eq:DPconj}
\DPGI{\T \psi}{\phibar} = \DPGI{\T \phi}{\psibar} \tfa \phi,\psi \in \HhGI,
\eeq
and
\beq\label{eq:adjoint1}
\aGs(u,v)= \LE(v) \quad\tfa v\in \HozDD,
\eeq
then $\overline{u}$ satisfies
\beq\label{eq:adjoint2}
a(\overline{u},v)= \overline{\LE(\overline{v})} \quad\tfa v\in \HozDD.
\eeq
\ele
...
\beq\label{eq:DtN}
\big\langle \DPGR{\DtN\psi}{\phibar} = \DPGR{\DtN \phi}{\psibar} \quad\tfa \phi,\psi \in H^{1/2}(\GR).
\eeq
...
\Cref{cor:1,cor:1a} are proved under the following two \lcnamecrefs{cond:1nbpc}.
...
(i) Given $k_0>0$, $h$ and $p$ are such that, given $f\in \LtD$, the solution $u_h$ of \cref{prob:fevgen} with $\aG = \aGo$ (and with $\LG(v)$ as defined in \cref{eq:LGf}) exists and is unique for all $k\geq k_0$. % (so that the matrix $\Amato$ is invertible). 
Furthermore, if $f= n\sum_j \alpha_j\phi_j$ for some $\alpha_j \in \CC$ and  $n\in \LiDR$  (i.e.~$f$ is an arbitrary element of $\Vhp$ multiplied by $n$), then
\beq\label{eq:bound3}
...
(ii) Given $k_0>0$, $h$ and $p$ are such that, given $\LG\in \HozDDs$, the solution $u_h$ of the Galerkin method \cref{eq:fevgen} 
with $\aG = \aGo$
exists, and is unique for all $k\geq k_0$.
%(so that the matrix $\Amato$ is invertible). 
Furthermore, if $\LG(v)= (A\nabla \widetilde{f},\nabla v)_{\LtD}$, where $A\in L^\infty(D, \RR^{d\times d})$, $A$ is symmetric, and $\widetilde{f} \de \sum_j \alpha_j \phi_j$ with $\alpha_j\in \CC$
 (i.e.~$\widetilde{f}$ is an arbitrary element of $\Vhp$), then
\beq\label{eq:bound4}
\N{u-u_h}_{\HokD} \leq C^{(1)}_{\rm FEM2}\,k\, \N{\LE}_{\HokDs} \quad\tfa k\geq k_0, 
\eeq
where $C^{(1)}_{\rm FEM2}$  is independent of $k$ and $h$, but dependent on $\Aso, \nso, D, R, k_0$, and $p$.
\end{condition}

For details of when \cref{cond:1nbpc,cond:2} are satisfied (for \cref{prob:vedp,prob:vtedp}), see \cref{sec:pdetheory} (for \cref{cond:1nbpc}) and \cref{sec:helmfedisc} (for \cref{cond:2}).

\bre[\Cref{eq:bound4} has the same $k$-dependence as \cref{eq:bound3}]\label{rem:yesitis}
Note that \cref{eq:bound4} has the same $k$-dependence as \cref{eq:bound3} despite the fact that a factor $k$ appears on the right-hand side. If $\LE(v) = \int_{D} f\vbar,$ then
\beqs
\NHokDs{\LE} = \sup_{v \in \HozDD} \frac{\abs{\LE(v)}}{\NHokD{v}} \leq\sup_{v \in \HozDD} \frac{\NLtD{f}\NLtD{v}}{\NHokD{vz}} \lesssim \frac1k \sup_{v \in \HozDD} \frac{\NLtD{f}\NLtD{v}}{\NLtD{vz}} = \frac{\NLtD{f}}k.
\eeqs
...
C_2 
%\frac{m_+}{m_-} \left[ C_{\rm FEM1}^{(1)} + C_{\rm bound}^{(1)}\right] 
\left(\frac{m_+}{m_-}\right) \frac{\N{n}_{L^\infty(D)}}{k}
\eeq
for all $k\geq k_0$,
where
\beq\label{eq:C2}
...
Assume that \cref{cond:1nbpc} holds, and assume that Part (ii) of \cref{cond:2} holds. Then, for $A\in L^\infty(D,\RR^{d\times d})$,
\beq\label{eq:keybound2}
\max\Big\{\big\| (\Amato)^{-1} \Smat_A \big\|_{(\Dmat_k)^{-1}}, \,\,
\big\| \Smat_A (\Amato)^{-1} \big\|_{\Dmat_k}\Big\} \leq C_1\, k\N{A}_{L^\infty(D)}
\eeq
and
\beq\label{eq:keybound2a}
\max\Big\{\big\| (\Amato)^{-1} \Smat_A \big\|_2, \,\,
\big\| \Smat_A (\Amato)^{-1} \big\|_2\Big\} \leq C_1\,\left(\frac{s_+}{m_-}\right) \frac{1}{h}\N{A}_{L^\infty(D)}
\eeq
%\begin{align}\nonumber
%&\max\Big\{\big\| (\Amato)^{-1} \Smat_A \big\|_2, \,\,
...
\frac{
\big\|\matrixC^* \wvec \big\|_{\Dmat_k}
}{
\big\|\wvec\|_{\Dmat_k}
}
\eeq
where $\wvec \de (\Dmat_k)^{1/2}\vvec$, and where $\matrixC^*$ is the conjugate transpose of $\matrixC$ (i.e.~the adjoint with respect to $(\cdot,\cdot)_2$).
...
=
\frac{\NDk{\mleft(\AmatoI\Mmatn\mright)^* \wvec}}{\NDk{\wvec}}
= 
\frac{
\big\|((\Amato)^*)^{-1}\Mmat_n\wvec\big\|_{\Dmat_k}
}{
\N{\wvec}_{\Dmat_k}
},
 \eeqs
...
In a similar way to the proof of \cref{lem:keylemma1}, given $\fvec \in \CC^N$ and a symmetric $A\in L^\infty(D, \RR^{d\times d})$, let $\widetilde{f} \de \sum_j f_j \phi_j$ and observe that $\widetilde{f} \in \HozDD$. Define $\widetilde{u}$ to be the solution of the variational problem 
\beq\label{eq:411a}
a^{(1)}(\widetilde{u},v)= \widetilde{\LE}(v) \quad\text{ for all } v\in H^1(\Omega),
\quad\text{ where } \quad
 \widetilde{\LE}(v) \de(A\nabla\widetilde{f},\nabla v)_{L^2(\Omega)}.
\eeq
Observe that the definition of the norms $\|\cdot\|_{(\HokD)'}$ \cref{eq:dualnorm} and $\|\cdot\|_{\HokD}$ \cref{eq:weightednorm} and the Cauchy-Schwarz inequality imply that
\begin{align}
\big\| \widetilde{\LE}\big\|_{(\HokD)'}&\leq \big\|A\nabla \widetilde{f}\big\|_{\LtD}\nonumber\\
...
\frac{1}{\min\{\Asomin,\nsomin\}}\left( 1 + 2 C^{(1)}_{\rm bound}\nsomax k  \right) 
\right]\big\|\widetilde{\LE}\big\|_{(\HokD)'},\\
&\leq C_1 \, k\, 
...
By the assumptions on $\Aso,\, \Ast,\, \nso,\,\st$, and $\Dm$, $u^{(1)}$ and $u^{(2)}$ exist, are unique, satisfy $a^{(1)}(u^{(1)}, v) = \LE(v)$ for all $v \in \HozDD$ and $a^{(2)}(u^{(2)}, v) = \LE(v)$ respectively, where $\LE$ is given by \cref{eq:Ledp}. Subtracting these equations, we find that $u^{(1)}- u^{(2)}$ satisfies the variational problem
\beq\label{eq:vp1}
a^{(1)}(u^{(1)}-u^{(2)},v) = \widetilde{\LE}(v) \quad\tfa v\in H^1_{0,D}(D)
\eeq
where
\beqs
 \widetilde{\LE}(v)\de \int_{D} \left((\Ast-\Aso) \nabla u^{(2)}\right) \cdot\overline{\nabla v} + k^2 (\nso-\nst) u^{(2)}\overline{v}.
\eeqs
Now, by the Cauchy-Schwarz inequality and the definition of the norm $\|\cdot\|_{\HokD}$ (see \cref{eq:weightednorm}), we have
\begin{align*}
| \widetilde{\LE}(v)| &\leq \NLiDRRdtd{\Aso-\Ast} \big\|\nabla u^{(2)}\big\|_{L^2(D)}
\N{\nabla v}_{L^2(D)} 
\\& \hspace{5cm}+ k^2 
...
\end{align*}
(by Cauchy--Schwarz in $\RR^2$). Therefore, by the definition of the norm $\|\cdot\|_{(\HokD)'}$ \cref{eq:dualnorm},
\beqs
\big\|\widetilde{\LE}\big\|_{(\HokD)'}\leq \max\Big\{\big\|\Aso-\Ast\big\|_{L^\infty(D)}\,,\, \big\|\nso-\nst\big\|_{L^\infty(D)}\Big\}
\big\| u^{(2)}\big\|_{\HokD}.
\eeqs
...
\\
&\quad \left(\max\Big\{\big\|\Aso-\Ast\big\|_{L^\infty(D)}\,,\, \big\|\nso-\nst\big\|_{L^\infty(D)}\Big\}\right),
\end{align*}
...
\beq\label{eq:obs1}
(\Delta + k^2) \big(e^{i k r}\chi(r)\big) =  e^{i k r}\left(i k \frac{d-1}{r} \chi(r) + \Delta \chi(r)\right)=: -\widetilde{f}(r),
\eeq
where $\chi(r)$ is chosen to have $\supp \chi \subset D$. Observe that \cref{eq:obs1} is the Helmholtz operator applied to a circular wave $e^{ikr}$, with the added factor $\chi$ to ensure the function has compact support. The equation \cref{eq:obs1} can be proved using the formula for the Laplacian in $d$-dimensional spherical coordinates\ednote{Both---do you know a good `standard' reference for something like this? Currently the best I've found is the Wikipedia article on the Laplacian....} and observing that $e^{ikr} \chi(r)$ has angular derivatives equal to zero.

Observe that \cref{eq:obs1} proves the sharpness of the nontrapping resolvent estimate \cref{eq:bound1}, since both the $L^2(D)$ norm of $\widetilde{f}$ and the $\HokD$ norm of $e^{i kr}\chi(r)$ are proportional to $k$, and hence each to other (see, e.g., \cite[Lemma 3.10]{ChMo:08},  \cite[Lemma 4.12]{Sp:14}). Also, recall that the proof of the PDE bound \cref{eq:PDEbound} used the nontrapping resolvent estimate \cref{eq:bound1} applied to $\uso-\ust.$ Therefore we expect that if we set things up so that
\beq\label{eq:sharpdiff}
\uso-\ust = e^{ikr}\chi(r),
\eeq
because \cref{eq:obs1} shows the sharpness of the resolvent estimate \cref{eq:bound1} we expect that combining \cref{eq:obs1} and \cref{eq:sharpdiff} will show the sharpness of the PDE bound \cref{eq:PDEbound}. Moreover, the function $e^{ikr} \chi(r)$ was used to prove the sharpness of resolvent estimates in \cite[Discussion on p. 1445 and Lemma 3.10]{ChMo:08} and \cite[Lemma 4.12]{Sp:14}, and so we can expect it will also be effective for proving sharpness in our setting.

We now set things up so that \cref{eq:sharpdiff} holds. We define $\nso = 1$ and $\nst = \nso + c(k) \chitilde(r),$ for some function $\chitilde(r)$ such that $\widetilde{\chi}\in C^{\infty}(D)$, $\widetilde{\chi}\not = 1$ (so that $\nst\not = \nso$), $\supp \, \widetilde{\chi} \subset D$, and $\NLiDRR{\chitilde} = 1$ (so that $\NLiDRR{\nso-\nst} = c(k)$).   As above, let $\chi=\chi(r)$ with $\chi \in C^{\infty}(D)$ and $\supp \,\chi$ a compact subset of $D$. We will specify $\chitilde$ and $\chi$ in more detail later. Let $\ftilde(r)$ be as defined in \cref{eq:obs1}, and define
\beq\label{eq:obs3}
...
\eeqs 
and, since $\|\nso- \nst\|_{L^\infty(D)} = c(k)$, \cref{eq:sharp1} holds, as required.
...
%well-defined. 
Because $\chi$ and $\widetilde{\chi}$ are in $C^\infty(D)$, the only issue is what happens at the boundary of the support of $\widetilde{\chi}$, where $u^{(2)}$ has the potential to be singular.
...
\bre[\Cref{cor:1,cor:1a} are a special case of \cref{cor:1a,cor:1aalt}]
Observe that in the case $q=\infty$ \cref{cor:1a,cor:1aalt} become our previous results in the $L^\infty$ norm, \cref{cor:1,cor:1a}.
\ere
...
QMC methods (or rules) are high-dimensional quadrature rules, designed to give superior rates of convergence (with respect to the number of integration points) compared to Monte-Carlo methods. Suppose one wants to approximate $\EXP{Q},$ where $Q$ is some random variable (later in this \lcnamecref{sec:nbpcqmc}, $Q$ will be a function of the solution $u(\omega)$ of a stochastic Helmholtz equation). By definition, the expectation is
...
\eeq
i.e., the metric on $\cube{J}$ induced by the spatial $L^\infty$ norm. We use this metric to describe the geometry of our QMC points as the best rigorous results (in terms of $k$-dependence) are proved in the $L^\infty$-norm\footnote{Although, in line with the results in \cref{sec:weaknorm}, we could instead use a spatial $L^q$ norm, for some $q \geq 1$ in \cref{eq:dqmc}.}. Therefore, when considering which QMC points will give preconditioners suitable for use with other QMC points, this metric is a natural metric to use.


\subsubsection{A sequential algorithm}
We first describe a straightforward algorithm that uses nearby preconditioning to speed up QMC calculation for the Helmholtz equation. We call this a `sequential' algorithm because, unlike the `parallel' algorithm we describe below, it is intrinsically sequential and cannot be parallelised, i.e., finite-element solves for different realisations of the random field $n$ cannot be treated in parallel. Although, when performing the individual finite-element solves one is not restricted to a single core, i.e., one can use parallelisation  if the linear systems $\Amat$ are large enough to warrant this.
...
\While{$\Sremaining \neq \emptyset$}{
\eIf{GMRES applied to $\Lmat\Umat\Amat\bycurrent = \Lmat\Umat \bff$ converges in fewer than $\maxGMRES$ iterations}{
...
\paragraph{Surrogate metric} Whilst the metric $\dQMC$ is the metric related to the performance of nearby preconditioning (as described in \cref{sec:intronbpc} above), in practice $\dQMC$ is difficult to work with, since it is not obvious what geometry it induces on $\cube{J}$. Therefore, we work in an alternative, although related metric
  \beqs
...
\paragraph{Complexity} The sequential algorithm is simple and intuitive to describe, given it mainly revolves around `finding the nearest point'. However, the parallel algorithm is much more complicated, both in the underlying ideas, but also in its technical definition.
...
\begin{table}
  ...
  \end{table}
    ...
    In conclusion, we see that nearby preconditioning gives a significant speedup when applied to a QMC model problem.% We therefore expect that this technique will give significant speed up when applied to other, more realistic problems.
...
%% %% \een
    ...
    Having proved rigorous results on the effectiveness of nearby preconditioning, and also applied to it a UQ algorithm, we now review similar computational techniques in the literature. Whilst the idea of \emph{nearby} preconditioning in UQ (that is, choosing multiple realisations for which we calculate the preconditioner) is, as far as we are aware, novel, there has been a body of work on the closely-related idea of \emph{mean-based} preconditioning. Mean-based preconditioning is conceptually a simpler version of nearby preconditioning, where a single preconditioner is calculated corresponding to the mean of the random coefficients. Mean-based preconditioning has been most extensively studied for the stationar diffusion equation
    \beqs
\grad \cdot \mleft(a\grad u\mright)  = -f,
\eeqs
where a preconditioner corresponding to $\EXP{a}$ is used for all realisations. Both computational and mathematical results show that this technique is effective if the variance of $a$ is small enough, i.e., if most of the samples of $a$ are sufficiently close to $\EXP{a}$. However, at the end of this review we discuss two cases where mean-based preconditioning has been applied to the Helmholtz equation; these are the closest works in the literature to our work on nearby preconditioning.

Mean-based preconditioning was first developed in the context of so-called Stochastic Spectral Finite-Element Methods (SSFEM). In these methods, the random field $a$ is given by a series expansion, such as a Karhunen--Lo\`eve expansion, and the dependence of $u$ on the random parameters is given by a generalised Polynomial Chaos (gPC) expansion\optodo{Ref}. The resulting problem is then discretised in the whole space $D \times \Omega$, where $D$ is the spatial domain and $\Omega$ the probability space. The resulting discrete problems involve very large matrices of the form
\beq\label{eq:sgmatrix}
\Amat \otimes \Gmat,
\eeq
where $A$ is a standard finite-element matrix, $G$ is a matrix corresponding to the discretisation in $\Omega,$ and $\otimes$ is the Kronecker product.\optodo{ref} For SSFEMs (and the closely-related stochastic-Galerkin FEMs, see, e.g. \cite{BaNoTe:04}, which also have discretisations of the form \cref{eq:sgmatrix}) a mean-based preconditioner is a matrix of the form
\beq\label{eq:mbsg}
\Amato \otimes \ImatOmega,
\eeq
where $\ImatOmega$ is the identity matrix associated with the discretisation on $\Omega.$ Using mean-based preconditioner, of the form \cref{eq:mbsg}, gives considerable computational savings, as only one preconditioner of a standard finite-element matrix need be calculated.

When stochastic Galerkin methods are used with so-called `doubly-orthogonal bases'\optodo{ref}, then the linear system \cref{eq:sgmatrix} decouples into many distinct standard finite-element matrices; mean-based preconditioning has also been investigated in this context (and in the context of stochastic collocation methods, where one similarly obtains many different standard finite-element matrices) as will be discussed below.

The main insight gleaned from studies of mean-based preconditioning is that, as stated above, provided the variance of $a$ (or the equivalent stochastic coefficients, for other problems, is sufficiently small, then mean-based preconditioning is effective. We now give a brief overview of the literature for mean-based preconditioning.

\optodo{Do all the refs}The initial computational work on mean-based preconditioning for the stationary diffusion equation was carried out by Ghanem and Kruger, Pellissetti and Ghanem and Keese, with theory (proving bounds on the eigenvalues of the preconditioned matrices following from Powell and Ullmann and Ernst, Powell, Silvester, and Ullmann. All of the above results were for $a$ given by a (real or artificial) Karhunen--Lo\`eve expansion; that is, in the case where $a$ depends linearly on the noise. In the case where $a$ is a lognormal random field, Powell and Ullmann\optodo{10} declared mean-based preconditioners to be ineffective, and so developed more advanced preconditioners; in contrast, Ullmann, Elman and Ernst transformed a stationary diffusion problem with lognormal coefficient into a stationary convection-diffusion problem with a random coefficient depending linearly on the noise, before proving eigenvalue bounds as before.

With a more computational slant, Tipireddy, Phipps and Ghanem and Rooseel and Vandewalle compare the computational properties of several mean-based preconditioners and Elman, Miller, Phipps and Tuminaro compare the computational cost of mean-based preconditioners for stochastic Galerkin and stochastic collocation methods.

Seeking to apply mean-based preconditioning to more challenging problems, Powell and Silvester performed computational investigations into mean-based preconditioners for stochastic Galerkin discretisations of the steady-state Navier--Stokes equations, and Sousedik and Elman introduced a Gauss--Seidel-type preconditioner, using mean-based ideas, for the steady-state Navier--Stokes equations. Finally, Khan, Powell, and ilvester applied mean-based preconditioning to stochastic Galerkin discretisations of the eqautions for nearly-incompressible elasticity.

The two works applying mean-based preconditioning to many individual systems (for the stationary diffusion equation) are those of Eiermann, Ernst and Ullmann and Gordon and Powell; the former being computational results in a stochastic Galerkin setting; the latter being rigorous eigenvalue bounds in a stochastic collocation setting. Both works assume linear dependence on the noise, and show that mean-based preconditioning works well when the variance is sufficiently small.

The first work on mean-based preconditioning for the Helmholtz equation that we will discuss is the recent work of Wang and Liao. They discretise a stochastic Helmholtz problem with $k=10$ and $n$ given by a truncated Karhunen--Lo\`eve expansion (with either 4 terms or 1 term). Their discretisation uses a gPC expansion for the solution $u$, and whilst they use mean-based preconditioning (in the `Kronecker product' sense) they are more interested in investigating the effect of the number of terms in the gPC expansion on the accuracy of the discrete solution. Nonetheless, they see convergence using the mean-based preconditioner, although more iterations are needed when the random field is `close to' a resonant frequency\optodo{Ref}.

The work most similar to ours is the work of Jin and Cai, who use a stochastic Galerkin discretisation with a doubly-orthogonal basis, resulting in around 5000 linear systems. They take a Karhunen--Lo\`eve expansion for both $A$ (which is scalar-valued) and $n$ with 4 terms (with randomness given by $\Unif(-45\sqrt{3},45\sqrt{3})$ random variables) and with $k = 225.$ Their mean-based preconditioner is a 1-level additive Schwarz preconditioner, and they compare resuing the preconditioner with reusing the Krylov subspaces (as in, e.g., \optodo{ref}), as well as combining both techniques. Intriguingly, they see no additional benefit from reusing the preconditioner (but considerable benefit from recycling the Krylov subspaces). Based on our results in this \lcnamecref{chap:nbpc}, we can conjecture that they see no benefit because a single mean-based preconditioner because $k$ is reasonably large, and therefore for most of the realisations $\nsj$ of the random field, $k\NLiDRR{\EXP{n}-\nsj}$ is not sufficiently small, and so there is little-to-no effect on the number of GMRES iteration from mean-based preconditioning. We conjecture that if they had used multiple preconditioners spaced around the stochastic parameter space, they would have seen the computational improvements, as described in this \lcnamecref{chap:nbpc}.\optodo{This will need some work.}
...
If we apply \cref{cor:1a} to the \cref{eq:stopc} we can straightforwardly conclude the following \lcnamecref{cor:stonbpcas}

\bco[Almost-sure nearby preconditioning]\label{cor:stonbpcas}
Let $0 < \eps < 1,$ $\nst:\Omega \rightarrow \LiDRR$ satisfy the assumptions at the start of \cref{sec:hh-results}, $\no, \,\Dm,$ and $f$ be as in \cref{sec:problem}, and let the assumptions of \cref{cor:1a} hold. Then $\GMRES{\eps}{\nso}{\nst}$ is bounded independently of $k$ almost surely if
\beq\label{eq:nbpcas}
...
\item\label[itemdrawback]{it:notideal1} The condition \cref{eq:nbpcas} must hold almost surely, and
  \item\label[itemdrawback]{it:notideal2} \Cref{cor:stonbpcas} does not give any explicit information on the distribution of the number of GMRES iterations depends on the distribution of $\NLiDRR{\nso-\nst}.$
    \een
    ...
    To correct the deficiencies described in \cref{rem:notideal} one would aim to  prove a bound on the number of GMRES iterations depending explicitly on $\NLiDRR{\nso-\nst(\omega)}$, and then use this bound to prove a probabalistic estimate for the number of GMRES iterations. Such a bound is given in \cref{lem:probgmres1} in \cref{app:probgmres}. However, such a bound will be highly pessimistic, and will impart little useful information. The reason for this is that the Elman estimate \cref{cor:GMRES_intro} above (when applied to the nearby-preconditioned system $\AmatoI\Amatt$) only applies when $k\NLiDRRdtd{\Aso-\Ast}$ and $k\NLiDRR{\nso-\nst}$ are sufficiently small (as we saw in \cref{cor:1} above). Therefore one can only obtain detailed information of how the number of GMRES iterations depends on $\NLiDRRdtd{\Aso-\Ast}$ and $\NLiDRR{\nso-\nst}$ when these quantities are small (informally, when they are $\lesssim 1/k$). In all other cases (again, informally, when these quantities are $\gtrsim 1/k$) the only statement one can make about the convergence of GMRES is that there will be at most $N$ iterations, where $N$ is the number of degrees of freedom (this result is recalled in \cref{cor:gmresguaranteed}). In summary, current results on GMRES convergence will only allow us to prove very pessimistic bounds on how the number of GMRES iterations for $\AmatoI\Amatt$ depends on $\NLiDRRdtd{\Aso-\Ast}$ and $\NLiDRR{\nso-\nst}$. For completeness, we record these results in \cref{app:probgmres} below.
...
\subsection{Numerical probabailistic results for nearby preconditioning}\label{sec:qualgmres}

Notwithstanding the fact that we are limited in the probabilistic results we can \emph{prove} about nearby preconditioning, we will now see that we observe reasonable probabailistic behaviour when we perform numerical experiments. We again recall that (informally) \cref{cor:stonbpcas} states that we obtain almost-surely bounded GMRES iterations if $\NLiDRR{\nso-\nst} \lesssim 1/k.$ A sensible probabalistic analogue of this result would be that we have bounded \emph{average} number of GMRES iterations if the standard deviation of $\NLiDRR{\nso-\nst}$ is of ther order $1/k$. We can expect this result as the standard deviation of a random variable has the same units as the random variable itself, and therefore it is reasonable to assume that the same scaling as in \cref{cor:stonbpcas} will give a bounded average number of GMREs iterations. Will we see exactly this behaviour in our numerical experiments.
...
\item\label[itemcase]{it:sigma2} $\displaystyle \sigma  = \frac{1}k,$ and
  \item\label[itemcase]{it:sigma3} $\displaystyle \sigma  = \frac{1}{k^2}.$
    ...
    Based on the reasoning above we expect that in \cref{it:sigma2} the probability \cref{eq:gmresprob} \emph{is constant} as $k$ increases, and using similar reasoning, we expect that in \cref{it:sigma1} the probability \cref{eq:gmresprob} \emph{decreases} as $k$ increases and in \cref{it:sigma3} the probability \cref{eq:gmresprob} \emph{increases} as $k$ increases.   This is exactly the behaviour we observe in \cref{fig:prob-plot-0.0,fig:prob-plot-1.0,fig:prob-plot-2.0}. This behaviour demonstrates that whilst the theory developed in the rest of this \lcnamecref{chap:nbpc} does not allow us to \emph{prove} results about the probabalistic behaviour of nearby preconditioning, the theory does give us \emph{intuition} as to what the probabilistic behavour will be.
