...
\enth
\optodo{`Proofs on pages' everywhere}
...
Let $\Dm$, $\Aso$, and $\nso$ satisfy \cref{cond:1nbpc} below (so that $\uso$ exists, is unique, and satisfies a $k$-independent a priori bound), and let $\Dm$, $\Ast$, and $\nst$ be such that $u^{(2)}$ exists
