...
For $\Az \in \spd,$ we write $\NopCCd{\Az}$ for the operator norm induced by the Euclidean vector norm on $\CCd$ (i.e., $\NopCCd{\cdot}$ is the spectral norm), and for $\Az:\DR \rightarrow \spd,$ we write $\NLiDRRRdtd{\Az}$ for the norm $\NLiDRRR{\Nop{\Az(\bx)}}.$ Observe that $\NLiDRRRdtd{\cdot}$ is a slight abuse of notation, as $\LiDRRRdtd$ is not a vector space. Nevertheless $\NLiDRRRdtd{\cdot}$ still fulfils the properties of a norm on $\LiDRRRdtd.$ We define $\NWoiDRRRdtd{\cdot}$ similarly.
