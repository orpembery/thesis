...
For $\Az \in \spd,$ we write $\NopCCd{\Az}$ for the operator norm induced by the Euclidean vector norm on $\CCd$ (i.e., $\NopCCd{\cdot}$ is the spectral norm), and for $\Az:\DR \rightarrow \spd,$ we write $\NLiDRRRdtd{A}$ for $\NLiDRRRsmall{\Nop{A(\bx)}}.$ We write $\Do \compcont \Dt$ if $\Do$ is a compact subset of the open set $\Dt.$ Let $\OFP$ be a complete probability space. Throughout this \lcnamecref{chap:stochastic}, unless stated otherwise we equip a topological space with its Borel $\sigma$-algebra. See \cref{app:mtBs} for a summary of the measure-theoretic concepts used in this \lcnamecref{chap:stochastic}. Let
\bit
