To summarise the constants used in \cref{sec:fem}, we use the following table, where the constants are given in logical order (i.e., constants lower in the table will only depend on constants higher in the table\ednote{This isn't the case yet.}). As well as giving the definitions of the constants, we also state the place (Theorem, Lemma, etc.) where they are defined. If a constant is not defined in terms of other constants, but rather is given in the statement of a Theorem or Lemma (as for, e.g., the definition of $\CMT$ in \cref{thm:multiplicativetrace}), then the `Definition' column is left blank.
...
& \cref{lem:shiftnegativew}\\
  ...
  \frac{4\CSZfn{s+2}\CAfn{s}\Cprod{s}\NLiDop{A}^2}{\min\set{1,\frac1{\CP^2}}\Amin}, & m \in (1,\frac{d}2]\\
\frac{2\CSZfn{s+2}\CAfn{s}\CBanfn{s}\NLiDop{A}^2}{\min\set{1,\frac1{\CP^2}}\Amin}, & m \in (\frac{d}2,p-1]
  \end{dcases}
  $
& \cref{lem:ellprojerr} \\
  ...
  $\CcorLt$&$4\max\set{\CFEMo,\CFEMt}\mleft(\Cfn{0,1} + \Cfn{-1,1}\mright) \max\set{\CLtboundo,\CLtboundt}$&\Cref{thm:fembound}\\
  $\CcorHo$&$8\max\set{\CFEMo,\CFEMt}\mleft(\Cfn{0,1} + \Cfn{-1,1}\mright)\max\set{\CHoboundo+\CLtboundo,\CHoboundt+\CLtboundt}$&\Cref{thm:fembound}\\
