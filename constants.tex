...
\caption[]{(continued)}\\
  ...
  $\Cmclean{m}$&---&Proof of \cref{thm:banachalg}\\
    $\CBanfn{m}{\mtilde}$ & --- & \cref{thm:banachalg}\\
%  $\Cprod{m}$ &$1+\Cmclean{m}$&\cref{thm:banachalg} \\
$\Cmult$ & $\max_{m=2,\ldots,p-2} \CBanfn{m}{\max\set{p-1,\ceil{d/2}+1}}$ & Proof of \cref{thm:expansion}\\
  $\Cej$ & $\displaystyle \begin{dcases}
  \max\CAz, & j=0\\
  \max\set{1,\CAo}\mleft(1+\CTrfn{2}\mright)\Cefn{0}, &j = 1\\
  \CAfn{j}\mleft(1+\CTrfn{j+1}\mright)\max\set{\Cmult \Cefn{j-2},\Cefn{j-1}} & j \in [2,p-2]
...
$\Cremfn{j}$ & $ \displaystyle \begin{dcases}
\CAfn{1}\mleft(1+\CTrfn{2}\mright), &j=1\\
\CAfn{2}\mleft(1+\CTrfn{3}\mleft(1+\Cremfn{1}\mright)\mright), &j=2\\
\CAfn{j}\mleft(1+\CTrfn{j+1}\mleft(\Cremfn{j-1}+\Cremfn{j-1}\mright)\mright), &j=3,\ldots,p-1
\end{dcases}$& Proof of \cref{thm:expansion}\\
  $\Cosc$ & $\Cremfn{p-1}$ & \cref{thm:expansion}\\
  ...
  \begin{dcases}
    \frac{2\NLiDop{A}}{\min\set{1/\frac1{\CP^2}}\Amin}, & m = -1\\
      \NLiDop{A}\CSZfn{2}\CAfn{0}\Cmmofn{-1}, & m = 0\\
      \CSZfn{m+2} \CAfn{m} \NLiDop{A}\Cmmofn{-1}, & m \in \mleft[1,p-1\mright]
  \end{dcases}
  $
& \cref{lem:ellprojerr} \\
  $\Cwmm$ &$\displaystyle
  \begin{dcases}
\Cmmofn{m}, & m = -1,0\\
\CBanfn{m}{\max\set{p-1,\ceil{d/2}+1}}\Cmmofn{m}, & m \in \mleft[1,p-1\mright]
  \end{dcases}
  $
& \cref{lem:ellprojerr} \\
% Insert C_weight in here
  ...
  1/\Amin, & m=-1\\
  \CAz, & m = 0\\
  2\CAfn{m}\CBanfn{m}{\max\set{p-1,\ceil{d/2}+1}}, & m \in \mleft[1,p-1\mright]\\
\end{dcases}
$
& \cref{lem:shiftnegativew}\\
  ...
  $\CReco$&$\displaystyle\sqrt{\frac32} + \CBo^{\half} + \frac{\CBt}{\sqrt{2}}+ \frac{\CBth}{\sqrt{2}}$&Proof of \cref{lem:higherbound}\\
  ...
  $\Cfourteen$ &$2\Csumfn{p-1}\sum_{m=0}^{p-1} \mleft(\CSZfn{p-1-m}\Cmmofn{0} + \Cwfn{p-1-m} \Cmmofn{-1}\mright)$&Proof of \cref{lem:ltthetahbound}\\
    $\Cc$&$\displaystyle2\max\set{\NLiDop{A},1,\frac{\CMT^2}2}$&\cref{lem:continuity}\\
  $\Cfirst$ &$\Cc\mleft(\Cmmofn{0} + 2\Cmmofn{1}\mright) + \CMT \CinvVhp \Cmmofn{0} \Cboundaryt^{\half} + \frac{\Cmmofn{0}}{\CFEMo}$&Proof of \cref{lem:ltthetahbound}\\
