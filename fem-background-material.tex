...
All results showing quasi-optimality for the Helmholtz equation (for different finite-element spaces and different domains) can then be seen as simply obtaining estimates on $\wbaadj$ in these different scenarios; this is the literature is summarised in \cref{tab:qo}. For example, if $p=1,$ then one can show $\wbaadj \sim hk,$ and hence we can conclude the first-order finite-element method is $\hka{2}$-quasi-optimal.
...
\begin{align}
  ...
  &\quad\quad\text{ by \cref{eq:epho}, \cref{eq:eplt}, \cref{eq:aepnormequiv}, and \cref{eq:wbaimpl}.}\nonumber
\end{align}
...
\eeq
where $\Nfn{p-1,h}{\cdot}$ is a discrete norm analogous to the Sobolev norm of order $p-1$. The bounds \cref{eq:duwu1,eq:duwu2} are then combined to show
\beqs
...
\Cref{it:ban1} is given in \cite[Section 1.8.1, Theorem]{Ma:11}. For \Cref{it:ban2}, observe that since $1-\vo$ has compact support, $\vo \in \WmiD,$ and $1-\vo \in \CmcompD$. Therefore, by \cite[Theorem 3.20]{Mc:00}, there exists $\Cmclean{m} > 0$ such that\footnote{In \cite[Theorem 3.20]{Mc:00} $\Cmclean{m}$ is denoted $C_{m}$.}
\beqs
\NHmD{(1-\vo)\vt} \leq \Cmclean{m} \NWmiD{1-\vo}\NHmD{\vt}.
\eeqs
Therefore since $\NHmD{\vo\vt} \leq \NHmD{\mleft(\vo-1\mright)\vt} + \NHmD{\vt} = \NHmD{(1-\vo)\vt} + \NHmD{\vt}$, we have \cref{eq:ban2}.
\epf
...
We only prove \cref{eq:feipsplit}, as the proof of \cref{eq:feiptrans} is analogous. Since $\vh,\wh \in \Vhp,$ there exist sequences $(\aj)_{j =1,\ldots,\dimVhp}$, $(\bsl)_{l =1,\ldots,\dimVhp}$ such that $\vh = \sum_{j=1}^{\dimVhp} \aj\phij$ and $\wh = \sum_{l=1}^{\dimVhp} \bsl \phil.$ Then we have
\begin{align}
...
\beqs
\IPLtDn{\vh}{\wh} \leq \Nmhn{\vh}\Nmmhn{\wh}.
\eeqs
\eco
...
\eeq
We first bound the negative norm $\Nfn{1-p,h}{\Qhn\rho}$, to do this we use \cref{lem:negdiscsum}. However, since we will apply \cref{lem:negdiscsum} we need to estimate negative (standard) Sobolev norms of $\Qhn\rho$; for integers $m \in [0,p-1]$ we have (observing that $\Qhn \Ph u = \Ph u$ since $\Ph u \in \Vhp$).
\begin{align}
...
Therefore since $\eh = \rho + \thetah$ we can rearrange \cref{eq:doublego} and use the Cauchy-Schwarz inequality to obtain
\begin{align}
