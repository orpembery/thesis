...
Since $\Ph$ is a Galerkin projection, one can show that in its energy norm $\Nep{\cdot}$ (equivalent to the $k$-weighted $H^1$ norm $\NW{\cdot}$, with equivalence constants independent of $k$ via the mutliplicative trace inequality and the generalisation of the Poincar\'e inequality in \cite[Equation (6.16)]{Sp:15}) $\aep$ is coercive and continuous, and hence is quasi-optimal:
\beq\label{eq:epho}
