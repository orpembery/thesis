...
\item The only properties of $\Vhp$ that we use are the existence of a best approximation (see \cref{lem:scottzhang} below) and the existence of an inverse inequality (see \cref{lem:inverseinequality} below). Since one can prove an analogous best approximation result for isoparametric finite elements (see, e.g., \cite[Theorem 4.7.3]{BrSc:08}), and we expect one can also prove an analogous inverse inequality (c.f. the proof of the standard inverse inequality in \cite[Section 4.5]{BrSc:08} with the definition of isoparametric finite elements in \cite[Section 4.7]{BrSc:08}), we expect that our original results in \cref{sec:errbounds} below will also hold in the case of isoparametric finite elements.
  ...
  \bas[Conforming triangulation and subspace]\label{ass:conforming}
We work with a family of triangulations $\mleft(\Th\mright)_{h>0}$ and their corresponding finite-element spaces $\mleft(\Vhp\mright)_{h>0}$, where for any $h>0,$ $\Th$ is a triangulation of $D$ (in the sense of \cref{def:triangulation}), and $\Vhp$ as defined in \cref{def:Vhp} is a subspace of $\HoD.$
  \eas
...
In the following definitions, we consider \cref{prob:vtedp}, and its discretisation \cref{prob:fevtedp}, as parameterised by the wavenumber $k,$ that is, we write $u(k)$ for the solution of \cref{prob:vtedp} with wavenumber $k$, and $\uh(k)$ for its finite-element approximation, the solution of \cref{prob:fevtedp}.

\bde[$\hka{a}$-quasi-optimal]\label{def:hkqo}
Given $a > 0,$ we say that the $h$-finite-element method for the Helmholtz equation is \defn{$\hka{a}$-quasi-optimal} if, given $\kz > 0$, there exist $\Co(\kz), \Cqo(\kz) > 0$ such that if
\beqs
hk^{a} \leq \Co,
\eeqs
...
Given $a,b>0$, we say that the $h$-finite-element meth\-od for \cref{prob:vtedp} is \defn{$\hk{a}{b}$-data-accurate} if, given $0 < \eps < 1,$ and $\kz > 0,$ there exist $\Co(\kz),$ $\Ct(\eps,\kz) > 0$ such that if
...
\eeq
then the $h$-finite-element method is $\hk{(p+2)/(p+1)}{(2p+1)/(2p)}$-data-accurate.
...
Given $a,b>0$, we say that the $h$-finite-element method for the Helmholtz equation is \defn{$\hk{a}{b}$-accurate} if, given $0 < \eps < 1,$ and $\kz > 0,$ there exist $\Co(\kz),$ $\Ct(\eps,\kz) > 0$ such that if
\beq\label{eq:hkacca}
hk^a \leq \Co
\eeq
and
...
We now use a modified version of the standard Aubin--Nitsche duality argument to bound $\NLtD{u-\uh}$ by $\wbaadj \NHokD{u-\uh}$ (and recall that $\wbaadj$ can be made small). Let $\xi \in \HozDD$ solve the adjoint Helmholtz problem
\beq\label{eq:xidef}
...
Since we are using higher-order finite elements (which we assume are of degree $p$), the arguments we use to obtain full convergence rates will require smoothness assumptions on $A$, $n,$ and the boundaries $\GD$ and $\GI$. We also make simple assumptions on $k,$ $\nmin$, and $\NLiDRR{n}$ in order to simplify our calculations. We first recall the definition of quasi-uniformity (c.f. \cite[Definition 4.4.13]{BrSc:08}):
...
\eeqs
\begin{align}
  \CLtn(n) &= \mleft(\mleft(\En\nvar\mright)^{\half(\floor{\frac{p-1}2}+1)}\nmax\nmin^{-\frac{p}2}\mright)^{p-1} \nvar^5 \nmin^{-(p+1)} \mleft(\En\nvar\mright)^{\half(\floor{\frac{p-1}2}+1)} \Pfn{p-2}(n)^2\nonumber\\
  &\quad\quad+ \Pfn{p-2}(n)\label{eq:CLtn}
\end{align}
and
\begin{align*}
...
\begin{align*}
  ...
  &\leq \CAj \mleft(1+\CTrjpo\mright)\Big(\Cmult\Cefn{j-2}\SNtemp{n}\Pfn{j-2}(n)\\
  &\hphantom{\leq \CAj \mleft(1+\CTrjpo\mright)\big(}\quad\quad+ \Cefn{j-1}\Pfn{j-1}(n)\Big)\Cfg\text{, by induction,}\\
&= \Cefn{j} \Pfn{j}(n)\Cfg
\end{align*}
...
&  \NHnfn{-m}{D}{w-\Ph w} \leq\nonumber\\
  &\,\,\CSZfn{m+2}\CAfn{m} \CBanfn{m}{\max\set{p-1,\ceil{d/2}+1}} \NLiDop{A} \frac{\SNtemp{n}}{\nmin^2} h^{m+1} \NHoDn{w - \Ph w}.\label{eq:wep1pt1}
\end{align}
...
\eeq
We will be able to use \cref{eq:wlt,eq:who} to bound the terms involving $w - \Ph w$ in \cref{eq:boundingLtwo}.
\newpage
We first estimate the inner product terms in \cref{eq:boundingLtwo}:
\begin{align}
