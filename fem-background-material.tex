...
Also, we will require the following result, that $\HmD$ is a Banach algebra.\ednote{Owen to find a reference for this}\optodo{Find ref}

\bth[$\HmD$ is a Banach Algebra]\label{thm:banachalg}
...
\beqs
\NHsD{\vo\vt} \leq \CBanfn{s} \NHsD{\vo}\NHsD{\vt}.
\eeqs
\enth
...
\NHnfn{-s}{D}{v} = \sup_{w \in \HsD} \frac{\IPLtDn{v}{w}}{\NHsD{w}}.
\eeq
...
For any integer $s \in [d/2,p-1],$\optodo{Think \emph{very} carefully when you want to apply this} if $n \in \HsD$ then there exists a constant $\Cwmso >0$ such that for all $w \in \HozDD$
\beq\label{eq:ellprojerr}
\NHnfn{-s}{D}{w-\Ph w} \leq \Cwmso h^{s+1}\NHmD{n} \BAHoD{w}{\wh},
\eeq
where
\beqs
\Cmso = \frac{\NLiDop{A}^2}{\Amin}\CSZs.
...
The proof follows the proof in \cite[Theorem 5.8.3]{BrSc:08}. Let $\phi \in \HsD,$ and observe that by \cref{thm:banachalg} $n\phi\in \HsD$ also. Let $ \vtilde = \sdres(n\phi),$ observe that by \cref{thm:shift} $\vtilde \in \Hfn{}{m+2}{D}$. For all $v \in \HozDD,$ we have
\beqs
\NLtD{A \grad \vtilde}{\grad v} = \IPLtD{n\phi}{v} = \IPLtDn{\phi}{v}.
\eeqs
...
\beqs
\NHnfn{-m}{D}{w-\Ph w} \leq \CSZfn{m+2}\CAfn{m} \CBanfn{m} \NLiDop{A} \NHmD{n} h^{m+1} \SNHoD{w - \Ph w},
\eeqs
...
%% \NHmsnD{w-\Qhn w} \leq \CSZfn{s} \NLiD{n} h^{s} \BALtDn{w}{\wh}.
%% \eeq
%% \ele

%% \bpf[Sketch Proof of \cref{lem:wltdprojerr}]
...
%% \begin{align*}
%% \IPLtDn{w-\Qhn w}{\vtilde} &\leq \NLiD{n}\NLtDn{w-\Qhn w}\NLtD{\vtilde-\vhptilde}\\
%% &\leq \CSZfn{s} \NLiD{n}\NLtDn{w-\Qhn w} h^s \NHsD{\vtilde}.
%% \end{align*}
%% Taking the supremum over $\vtilde,$ we have
%% \beqs
%% \NHmsDn{w-\Qhn w} \leq \CSZfn{s} \NLiD{n} h^s \NLtDn{w-\Qhn w},
%% \eeqs
...
\beq\label{eq:sdeq}
\grad \cdot \mleft(A\grad \utilde\mright) = -\ftilde \text{ in } D
\eeq
\beq\label{eq:sddbc}
\trD \utilde = 0
\eeq
...
If one chooses an orthonormal basis $\phij$ for $\Vhp,$ writes $\Delta\wh = \sum_j \wj \phij$, and takes $\vh = \phij$ for each $j,$ then we see \cref{eq:discderdef} is equivalent to the linear system $\Imat \bw = \bb,$ where $\bb_{j} = \IPLtD{A \grad \wh}{\grad \phij}.$ The solution of this linear system clearly exists and is unique.
...
In particular, one can think of $\DeltahI$ as being a `discrete resolvent', i.e., a discrete counterpart to $\sdres.$
...
To prove \cref{lem:negdiscsum} on the relationship between discete and continuous negative Sobolev normns, we require the following \namecref{lem:shiftnegativew} giving the shift theorem in negative weighted norms.
...
\beqs
\NHnfn{-m}{D}{\sdres\ftilde} \leq \Cshiftfn{m} \NHfn{m}{D}{n}\NHfn{m+2}{D}{\frac1n}\NHnfn{-(m+2)}{D}{\ftilde}.
\eeqs
\ele

\bpf[Proof of \cref{lem:shiftnegativew}]
Observe that by \cref{thm:banachalg}, if $v \in \HmD$ then $nv \in \HmD,$ and therefore by \cref{thm:shift} $\sdres(nv) \in \HmptD.$ Hence, by \cref{thm:banachalg} again, $\sdres(nv)/n \in \HmptD.$ We these facts in place we can compute
\begin{align*}
