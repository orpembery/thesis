...
\end{dcases}
\eeq
We use the notation $\En\max\set{\errn{m}\st m = -1,\ldots,p-1}.$
%% \ben
...
For the case $m \geq 1$, let $\phi \in \HsD,$ and observe that by \cref{thm:banachalg} $n\phi\in \HsD$ also. Let $ \vtilde = \sdsol(\phi),$ observe\ednote{$\Sn$ is defined in \cref{sec:discsob}---I haven't yet decided where to put the definition.} that by \cref{ass:highp,thm:shift} $\vtilde \in \Hfn{}{s+2}{D}$. For all $v \in \HozDD,$ we have
\beqs
