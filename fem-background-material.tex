...
For simplicity's sake, our exposition below will assume that we are treating the homogeneous Helmholz TEDP (i.e., $A=I$ and $n=1$) with $\gI = 0,$ and that the problem is nontrapping\optodo{Look up ref}. Also, we suppress all of the constants involved, instead opting to use $\lesssim$ notation, as defined in\optodo{Find where defined, or define here.} The new results we present in \cref{sec:fem} below relax all of these assumptions, and the price of being more technical to state.
\optodo{Maybe rework bounded interpolation error/best approx error and put it in here.}
...
\end{align}
Taking $\eps$ sufficiently small, and moving the first term in \cref{eq:epboundpart1} to the left-hand side, taking $\vh$ to be the best approximation of $u$ in $\Vhp,$ and using \cref{lem:scottzhang} and\optodo{$H^2$ bound anywhere?} we can show
\beqs
...
\eeqs
There exist obvious generalisations of these operators to the case with non-zero impedance boundary conditions. We next define the `worst best approximation' constants:
\beqs
...
\eeqs
We note that we are changing notation slightly from that normally prevelant in the literature; notation for $\wbaadj$ was first introduced by Sauter in \cite{Sa:06}, but $\wbaadj$ was instead denoted $\eta.$ When dealing purely with the Schatz argument for quasi-optimality (as in \cite{Sa:06}\optodo{Check}) one only needs consider `worst best approximations' adjoint problems in the duality-argument step, and hence one only needs notation for $\wbaadj$. However, in our exposition of elliptic-projection-based arguments below, we will also need to consider `worst best approximations' of non-adjoint\ednote{Both---Is there a word for this? `Standard'?} Helmholtz problems, and hence we adopt the notation above.

\paragraph{The Schatz argument} We now recap the (generalisation) of the Aubin--Nitsche duality argument for bounding the $L^2$-norm of the finite-element error by the $H^1$ norm of the error.\optodo{Check if this was done by Schatz, and add in here.} This recapitulation will allow us to see where $\wbaadj$ enters the argument, and will also allow us to compare and contrast this argument with the arguments used to obtain a bounded finite-element error using an elliptic projection.
...
\eeq
To obtain a bound on the error in the $H^1$ norm, we take $\vh = \Ih u$ in \cref{eq:gardingerror}, and analagous to \cref{eq:epboundpart1}, obtain for any $\eps > 0$
\beq\label{eq:ep2}
...
\begin{tabu}{ccc}
  ...
  \cite{MeSa:10,MeSa:11} & $h^{p}k^{p+1}$ or $\log(k)/p$ and $kh/p$ & Full-space problem, IIP, and EDP\\
  \cite{ChNi:19} & $h^pk^{p+\alpha+1}$ & Arbitrary problem, with assumption that priori bound grows at rate $k^\alpha$\\
  \cite{ChNi:18} & $hk^2$ & TEDP with re-entrant corners\\
  \cite{ChGaNiTo:19} & $h^{p}k^{p+1}$ & Truncated perfectly matched layer\\
\bottomrule
\end{tabu}
\optodo{Would it be better to put bounds on $\eta$ here?}
\optodo{Give more detailed lit review.}
