...
To bound the term $\NHfn{m}{D}{n\phi}$ in \cref{eq:ellprojwpart} we need to distinguish between the cases $m > d/2$ and $m \leq d/2$. If $m \in (d/2,p-1],$ by \cref{thm:banachalg,eq:nconv}, then \cref{eq:ellprojwpart} is bounded above by
...
&\quad\quad \mleft(\En \nvar\mright)^{\floor{\frac{p-1}2}+1}\nmax^2\Pfn{p-2}\mleft(\nmax\mright)\mleft(\CFEMotilde h + \CFEMttilde \CAnk (hk)^p\mright)\NLtD{\thetah}.\label{eq:inedits2}
\end{align}
...
\begin{align*}
  ...
  &\leq \Cmmofn{0} \max\set{\CFEMo,\CFEMt}\Pfn{p-2}\mleft(\nmax\mright)\mleft(h^2 +  \CAnk h\mleft(hk\mright)^{p}+ \CAnk^2\mleft(hk\mright)^{2p}\mright)\Cfg\\
