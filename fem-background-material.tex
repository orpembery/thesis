...
Observe that $\Ph w$ and $\Phn w$ are the finite-element solutions of stationary diffusion problems with `diffusion coefficient' A and particular right-hand sides in $\HozDD'.$
...
\end{align*}
and by applying \cref{thm:banachalg} to the term $\NHfn{m}{D}{nv}$ (or observing that $\NLtD{nv} \leq \NLiD{n}\NLtD{v}$, in the case $m=0$), the result follows.
\epf
\tohere
