...
\bre[\Cref{thm:fembound} is a higher-order result]
...
Recall that $\CAnk$ and $\Cfg$ are defined in \cref{ass:htwo}. The constants $\Cej$ and $\Cosc$ are defined in \cref{sec:constants}.
...
by using using \cref{lem:ellprojerr} with $m=1$ and \cref{lem:scottzhang} to bound $\NHokD{\rho}$ and  , , and using the fact that $hk \leq 1,$ we obtain \cref{eq:femltbound,eq:femhobound}.\optodo{Make this last line clearer}
\epf
\optodo{See Braess 3rd edn p. 239 for a defn of the discrete norm on the matrix level - is his definition the same as ours?}
