...
\NLtD{f} \lesssim \NW{u}.
\eeq
However, in general \cref{eq:reversebound} does not hold\optodo{Figure out if there's an easy counter example.}, and so one cannot, in general, conclude $\hk{a}{b}$-accuracy from $\hk{a}{b}$-data-accuracy.
\ere
...
The best results known to date are the same (in terms of $a$ and $b$) as those for $\hk{a}b$-accuracy, except results for $\hk{a}b$-data-accuracy hold in higher dimensions. In \cite[Theorem 5.1, Corollary 5.2]{DuWu:15} Du and Wu proved that the $h$-FEM for the Interior Impedance Problem (IIP) is $\hka{(2p+1)/(2p)}$-data-accurate for arbitrary (fixed) polynomial degree $p$ and $d = 2$ or $3$, provided $u \in \HppoD$ (although this result can be shown for lower-regularity solutions by combining \cite[Theorem 5.1]{DuWu:15} with \cite[Lemma 3.5]{DuWu:15}).\optodo{Mention somewhere that our results hold even if soln not smooth using Theo and Serge results.} This result was proved for the IIP in $d = 2$ or $3$ for $p=1$ by Wu in \cite{Wu:14}. Whilst there is no computational evidence to confirm the sharpness of this result, given it coincides with the (sharp) best case for $\hk{a}b$-accuracy, we expect the result for $\hk{a}b$-data-accuracy is also sharp.

\paragraph{$\hka{a}$-quasi-optimality} Finally, the best known result for $\hka{a}$-quasi-optimality for the Helmholtz equation is that the $h$-FEM is $\hka{(p+1)/p}$-quasi-optimal. This was first proved for $p=1$ in 1-d by Aziz, Kellog, and Stevens in \cite[Theorem 3.1]{AzKeSt:88} and Ihlenburg and Babu\v{s}ka in \cite[Theorem 3]{IhBa:95a} and \cite[Theorems 4.9 and 4.13]{Ih:98}, with \cite[Figures 7-9]{IhBa:95a} and \cite[Section 4.5.4 and Figures 4.11-4.12]{Ih:98} showing this result is sharp in 1-d (although Ihlenburg and Babu\v{s}ka only work in the $H^1$-semi norm). The result was proved for $p=1$ and $d=2$ for the IIP by Melenk \cite[Proposition 8.2.7]{Me:95}, and for higher-order finite elements (for the full-space problem, IIP, and EDP) by Melenk and Sauter in \cite[Corollary 5.6]{MeSa:10} and \cite[Theorem 5.8]{MeSa:11} respectively. This result was shown for a PML problem by Chaumont-Frelet, Gallistl, Nicaise, and Tomezyk in \cite[Theorem 5.1]{ChGaNiTo:18}, and extended to arbitrary time-harmonic wave propagation problems by Chaumont-Frelet and Nicaise in \cite[Theorem 2.15]{ChNi:19}, who showed that if the constant in the a priori bound \cref{eq:bgbound} grows like $k^\alpha,$ then the $h$-FEM is $\hka{(p+\alpha+1)/p}$-accurate. They give numerical experiments in \cite[Figure 8]{ChNi:19} showing the sharpness of $\hka{(p+1)/p}$-quasi-optimality for $p=1, 2$ for a heterogeneous IIP. 
\optodo{Check somewhere I've said before this we're assuming and impedance-like problem and nontrapping}

\paragraph{Complete summary of results in the literature} In \cref{tab:acc,tab:dataacc,tab:qo} all the proofs in the literature of $\hk{a}b$-accuracy, $\hk{a}b$-data-accuracy, and $\hka{a}$-quasi-optimality. We list these in chronological order, with any relevant comments in the `Notes' column. The `Proof technique' column details the method used in the proof---see \cref{sec:prooftechniques} below for an extended discussion of these techniques.
\optodo{Make arraystretch play nicely with makecell.}
%\renewcommand{\arraystretch}{1.0} % From https://en.wikibooks.org/wiki/LaTeX/Tables#Space_between_rows
...
& $\hka{a}$-quasi-optimality & Notes & Proof technique\\
  ...
  %? \cite{IhBa:95b}
  \cite[Theorems 4.9 and 4.13]{Ih:98} & $\hka{2}$ & \makecell{$d=1,$ $H^1$ seminorm,\\\cite[Theorem 4.9]{Ih:98} is \cite[Theorem 3]{IhBa:95a}} & \makecell{(Respectively)\\Schatz and error splitting\\using interpolant\\respectively}\\
  \cite[Proposition 8.2.7]{Me:95} & $\hka{2}$ & \makecell{$d=2,$\\Interior Impedance Problem (IIP),\\$D$ smooth and star-shaped or convex} & Schatz\\
  ...
  \cite[Theorem 5.1]{ChGaNiTo:18} & $\hka{(p+1)/p}$&$d=2$, EDP truncated with PML, posed in a ball  &Schatz\\
    ...
    \cite[Theorems 4.2 and 4.5, Remark 4.6(ii)]{GrSa:18} &$\hka{2}$ & IIP,\makecell{$D$ Lipschitz, star-shaped w.r.t. a ball,\\$n$ heterogenenous\\constants fully explicit in $n$}& Schatz\\
...
\begin{tabu}{cccc}
 ...
 & $\hk{a}b$-data-accuracy  & Notes & Proof technique\\
      \midrule
      \cite{ZhWu:13}& $\hk{(p+2)/(p+1)}{(2p+1)/(2p)}$   &\makecell{$d=2,3$, IIP,\\$D$ smooth,\\bounds obtained for $hp$-FEM, so fully $p$-explicit}& Modified Schatz\\
  \cite{LiWu:18}& $\hka{3/2}$   & $d = 1,2,3$, full-space problem truncated with PML, posed in a ball& Modified Schatz\\
  \cite{ChGaNiTo:18}&$\hk{(p+2)/(p+1)}{(2p+1)/(2p)}$  & $d=2$, EDP truncated with PML, posed in a ball& Modified Schatz\\
  \cite{ChNi:18}& $\hk{3/(1+\alpha)}{3/2}$   &\makecell{$d=2$, TEDP with re-entrant corners,\\$\alpha \in (1/2,1)$ related to strength of corner singularities}& Modified Schatz\\
   \cite{Wu:14} & $\hka{3/2}$  & $d=2,3$, IIP, $D$ a star-shaped polygon/polyhedron & Error splitting\\
  \cite{DuWu:15} & $\hka{(2p+1)/(2p)}$ &  $d=2,3$, IIP, $D$ smooth, star-shaped& Error splitting\\
\bottomrule
\end{tabu}
\caption{$\hk{a}b$-data-accuracy for $h$-finite-element discretisations of the Helmholtz equation}\label{tab:dataacc}
\end{table}
\end{landscape}
\optodo{Why does Chaumont-Nicaise see the magnitude of the a priori bound, but Melenk--Sauter doesn't?}
\begin{table}[h]
\begin{tabu}{ccccc}
  \toprule
  & Mesh Condition & Error bound & Notes & Proof technique\\
  \midrule
 
\bottomrule
\end{tabu}
\caption{$\hk{a}b$-data-accuracy for $h$-finite-element discretisations of the Helmholtz equation}\label{tab:dasaasc}
\end{table}
\optodo{Put a note somewhere that one can get accuracy/dta-accuracy from QO, but we won't do that here.}
% QO
% Melenk Sauter hp
% &  or $\log(k)/p$ and $kh/p$ ($hp$) & \\
%   \cite{IhBa:97} & hk^2/p suff small - THeorem 3.5 & \\
...
% data-acc
% Zhu and WU $\displaystyle \frac{kh}p \mleft(\frac{k}p\mright)^{\frac1{p+1}}$ ($hp$) - mesh cond - mod schatz

% accurate
%\cite{IhBa:97} Cor 3.2 (and result used in this) - error splitting using interpolant
