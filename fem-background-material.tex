...
This lack of coercivity stems from the fact that we can no longer apply the Poincar\'e inequality \cref{eq:poincare} to functions in $\HozDD,$ because such functions are no longer zero on a portion of $\d D$ with non-zero $d-1$-dimensional measure (see the statement of \cref{lem:poincare}). One can alternatively view this problem as the fact that the stationary diffusion equation with Neumann boundary conditions does not have a unique solution (as one can simply add a constant to any solution), and, in the case $\GD=\emptyset,$ the PDE corresponding to the elliptic projection is precisely such a stationary diffusion equation: \cref{eq:sd1,eq:sd3} (\cref{eq:sd2} does not apply, as $\GD = \emptyset$).

The remedy for this lack of coercivity is to change the sesquilinear form $\aep$ from \cref{eq:aepho} to either \cref{eq:aeplower} or \cref{eq:aepused} (this change corresponds to changing the PDE underlying the elliptic projection from \cref{eq:sd1,eq:sd3} to either \cref{eq:sd4,eq:sd6} or \cref{eq:sd7,eq:sd9} respectively). Either of these alternatives for $\aep$ is coercive in the weighted $H^1$ norm, and so the proofs of \cref{lem:eugp,lem:ellprojerr,lem:ellprojerrw} go through as before.

However, if one chooses to use \cref{eq:aepused} to define $\aep$, then one encounters the additional problem that one does not have a shift theorem, analagous to \cref{thm:shift} for the underlying PDE \cref{eq:sd7,eq:sd9}. Such a shift theorem is used in the proof of \cref{lem:ellprojerr,lem:ellprojerrw}. Whilst one could rewrite \cref{eq:sd7,eq:sd9} as
\beqs
\Delta w = F  \tin D \tand
\eeqs
\beqs
\dn w = ikw \ton \GI
\eeqs
and then use \cref{thm:shift} to obtain bounds analagous to \cref{eq:shift}, the constants in these bounds would be \emph{$k$-dependent}. This $k$-dependence means the resulting proof of $\hka{(2p+1)/(wp)}$-data-accuracy would not work. Whilst a shift theorem for \cref{eq:sd7,eq:sd9} that \emph{is} $k$-independent has been proved by Chaumont-Frelet, Nicaise, and Tomezyk \cite[Theorems 3.1, 4.3, and 5.1]{ChNiTo:18}, this is a lowest-order shift theorem (i.e., the analogue of \cref{thm:shift} for $l=0$) and no mention is made in \cref{ChNiTo:18} of an extension to higher order.

Therefore, in our case, the only reasonable course of action when considering the IIP is to use \cref{eq:aeplower} to define $\aep$, as one can then repeat the proof of \cref{thm:shift} almost verbatim (as the results from \cite{Mc:00} used in the proof of \cref{thm:shift} also hold for the PDE \cref{eq:sd4,eq:sd6}), and the proof of \cref{thm:fembound} proceeds as before. However, we observe that this course of action works in our case because our proof technique allows us to bound terms arising on the truncation boundary $\GI$ (see \cref{lem:boundarybound} below). However, if one uses a modified Schatz technique, it is not clear to us how one would bound terms arising on the impedance boundary. Therefore, these terms must be incorporated into the elliptic projection, and so one must use \cref{eq:aepused} to define the elliptic projection, with the proviso that, with current technology, one will be limited to proving bounds for first-order methods, due to the lack of a shift theorem for the underlying PDE, as outlined above. (We note in passing that it was precisely this usage of an elliptic projection given by \cref{eq:aepused} by Chaumont-Frelet and Nicaise in \cite{ChNi:18} that led to the work \cref{ChNiTo:18}.)
\ere
