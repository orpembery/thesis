...
\item $\gD \in \HhGD,$
  \item $\gI \in \LtGI,$
\item $n \in \LiDRR$ such that $\supp\mleft(1-n\mright) \compcont D$, satisfying \eqref{eq:nbounds} with $\Dp$ replaced by $D$, and
...
\eeq
 The G\r{a}rding inequality means $\Re{a(v,v)}$ is `coercive' if an appropriate multiple of the $L^2$-norm is added to it.
