...
\end{numcases}
 \enth
 \optodo{Say why maximum may be needed}
...
\bit
\item The errors in their approximation have different $k$-dependencies (see \cref{thm:fembound} and \optodo{Insert ref} below), and
...
We now let $u:\Omega\rightarrow\HokD$ solve the TEDP-analogue of \cref{prob:msedp}, and $\uhtilde:\Omega\rightarrow\Vhp$ solve the stochastic analogue of \cref{prob:fevtedp} (i.e., $\uhtilde$ solves \cref{prob:fevtedp} with coefficients $A(\omega)$ and $n(\omega)$, $\T = ik$, and meshsize $\homega$ pathwise). We assume\optodo{Make more prominent?} $\uhtilde$ is measurable.
...
\bas[Variance of $\Qhtilde$ constant]
...
\epf
\optodo{Put somewhere that could do probabilistic results}
\optodo{Put somewhere remark about not seeing coarse space in MLMC}
