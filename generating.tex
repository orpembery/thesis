...
A(\omega,\bx) = \Az(\bx) + \sum_{j=1}^\infty \Yj(\omega)\sqrt{\Lambdaj} \Psij(\bx)\quad\text{and}\quad n(\omega,\bx) = \nz(\bx) + \sum_{j=1}^\infty\Zj(\omega)\sqrt{\lambdaj} \psij(\bx) ,
\eeq
where:
\bit
...
\sum_{j=1}^\infty \sqrt{\Lambdaj}\NLiDRop{\Psij(\bx) - \mleft(\bx\cdot\grad\mright)\Psij(\bx)} \leq 2\delta\mu,
\eeq
...
\sum_{j=1}^\infty\sqrt{\lambdaj}\NLiDRRR{\psij(\bx) + \bx\cdot\grad\psij(\bx)} \leq 2\delta\mu,
\eeq
